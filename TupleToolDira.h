...
class TupleToolDira : public TupleToolBase, virtual public IParticleTupleTool {
 private:
  typedef ROOT::Math::SMatrix<double, 10, 10,
                              ROOT::Math::MatRepSym<double, 10> >
      SymMatrix10x10;

 public:
  /// Standard constructor
  TupleToolDira(const std::string& type, const std::string& name,
 ...
 private:
  StatusCode fillDIRAError(const LHCb::VertexBase* primVtx,
                           const LHCb::Particle* P, const std::string head,
                           Tuples::Tuple& tuple) const;
  void calculateDIRAError(const LHCb::Particle*, const LHCb::VertexBase*,
                          double&, double&) const;
