...
TupleToolBremInfo::TupleToolBremInfo(const std::string& type,
  ...
  declareProperty(
      "fullDST",
      m_dst = true);  // to switch off/on information not available on microDST
  m_parts.push_back("e+");
...
StatusCode TupleToolBremInfo::initialize() {
  ...
  if (!m_parts.empty()) {
    for (std::vector<std::string>::const_iterator p = m_parts.begin();
      ...
      if (pp) {
        m_pids.push_back(pp->pdgID().abspid());
      }
    }
  }
...
StatusCode TupleToolBremInfo::fill(const Particle*, const Particle* P,
  ...
  bool filltuple = true;
  if (P->isBasicParticle() && P->proto() != NULL &&
      P->proto()->track() != NULL) {
    ...
    filltuple &= tuple->column(prefix + "_HasBremAdded",
                               P->info(LHCb::Particle::HasBremAdded, 0.) > 0);
    filltuple &= tuple->column(
        prefix + "_BremMultiplicity",
        P->info(LHCb::Particle::HasBremAdded,
                0.));  // replace the multiplicity() method that need fullDST
    // brem info from rerunning the BremAdder, looking at the photons
    // This is not correct when running on an mDST - you look at a subset of the
    // total photons in the event.
    if ((int)P->info(LHCb::Particle::HasBremAdded, 0.) !=
            (int)brem.multiplicity() &&
        m_dst)
      Warning(
          "TupleToolBremInfo requires fullDST -  BremP and BremOrigin might "
          "not be reliable (Multiplicity is OK)",
          StatusCode::SUCCESS, 5);
    if (m_dst)
      filltuple &=
          tuple->column(prefix + "_BremP", brem.momentum());  // need fullDST
    if (m_dst)
      filltuple &= tuple->column(prefix + "_BremOrigin",
                                 brem.referencePoint());  // need fullDST
    ...
    int k = 0;
    for (LHCb::Particle::ConstVector::const_iterator itmp = daughters.begin();
         itmp != daughters.end(); itmp++) {
    ...
    double mult = p1->info(LHCb::Particle::HasBremAdded, 0.) +
                  p2->info(LHCb::Particle::HasBremAdded, 0.);
    const std::pair<LHCb::CaloMomentum, LHCb::CaloMomentum> brems =
        m_adder->bremMomenta(p1, p2);
    LHCb::CaloMomentum b1 = brems.first;
    LHCb::CaloMomentum b2 = brems.second;
    filltuple &= tuple->column(prefix + "_DiEBremMultiplicity", mult);
    if (m_dst)
      filltuple &=
          tuple->column(prefix + "_DiEBremP", b1.momentum() + b2.momentum());
    ...
    filltuple &= tuple->column(prefix + "_" + name1 + "_DiEBremMultiplicity",
                               p1->info(LHCb::Particle::HasBremAdded, 0.));
    if (m_dst)
      filltuple &=
          tuple->column(prefix + "_" + name1 + "_DiEBremP", b1.momentum());
    filltuple &= tuple->column(prefix + "_" + name2 + "_DiEBremMultiplicity",
                               p2->info(LHCb::Particle::HasBremAdded, 0.));
    if (m_dst)
      filltuple &=
          tuple->column(prefix + "_" + name2 + "_DiEBremP", b2.momentum());
    ...
    if (m_dst) {
      ...
      filltuple &=
          tuple->column(prefix + "_" + name1 + "_StandaloneBremMultiplicity",
                        sb1.multiplicity());
      filltuple &=
          tuple->column(prefix + "_" + name2 + "_StandaloneBremMultiplicity",
                        sb2.multiplicity());
    }
  }
