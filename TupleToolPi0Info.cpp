...
TupleToolPi0Info::TupleToolPi0Info(const std::string& type,
    ...
    : TupleToolBase(type, name, parent), m_Pi0ID(111) {
...
StatusCode TupleToolPi0Info::fill(const Particle*, const Particle* P,
  ...
  if (P) {
    if (P->particleID().pid() == m_Pi0ID && isPureNeutralCalo(P)) {
      debug() << " Make the special case for the pi0  " << P->particleID().pid()
              << endmsg;
      debug() << " The pi0s has : " << P->daughters().size() << " daughters  "
              << endmsg;
      int Type = 0;
      if (P->daughters().size() > 0) {
        Type = 2;  // for the  resolved pi0s
      } else
        Type = 1;  // for the merged pi0s
      ...
      if (m_RequireMCTruth == true) {
        Warning(
            "MC association with TupleToolPi0Info is deprecated - please use "
            "the standard TupleToolMCTruth",
            StatusCode::SUCCESS)
            .ignore();
        ...
        filltuple &=
            tuple->column(prefix + "_MCPi0Mother_key", MCPi0Mother_key);

        filltuple &=
            tuple->column(prefix + "_MCPi0GrandMother_id", MCPi0GrandMother_id);
        filltuple &= tuple->column(prefix + "_MCPi0GrandMother_key",
                                   MCPi0GrandMother_key);

        filltuple &= tuple->column(prefix + "_MCPi0GrandGrandMother_id",
                                   MCPi0GrandGrandMother_id);
        filltuple &= tuple->column(prefix + "_MCPi0GrandGrandMother_key",
                                   MCPi0GrandGrandMother_key);
        debug() << "This is the MC part associated to your  pi0 -->   "
                << MCPi0_id << "   and its  mother   -->" << MCPi0Mother_id
                << endmsg;
      }  // require MC truth
    }    // this is a pi0
  }      // get the particle
...
//================================================================================
TupleToolPi0Info::mcRange TupleToolPi0Info::getRange(
...
//================================================================================
// Here you get the relation table between the protoparticles and the MC
// particles
//================================================================================
TupleToolPi0Info::mcRange TupleToolPi0Info::getRange(
  ...
  if (0 == proto->charge()) {
    if (context() == "HLT") {
      debug() << " You are running in the Hlt context   " << context()
              << endmsg;
      locationhard = "Relations/Hlt/ProtoP/Neutrals";

    } else {
      debug() << "You are running in the Offline context   " << context()
              << endmsg;
...
//=======================================================================================
const LHCb::MCParticle* TupleToolPi0Info::getMCPi0(const LHCb::Particle* part,
                                                   double& weight) {
  // Return the highest-weighted matching MCPart WITH THE SAME particleID than
  // part->ID if not the highest-weighted among ALL matching MCPart then  weight
  // = -weight If not found return the highest-weighted matching MCPART
  // (irrespective of part->ID)
  ...
  if (part->daughters().size() > 0) {
    ...
    for (mcRange::const_iterator ir1 = range1.begin(); ir1 != range1.end();
         ++ir1) {
      for (mcRange::const_iterator ir2 = range2.begin(); ir2 != range2.end();
           ++ir2) {
        double w = (ir1)->weight();
        ...
        if (m_Pi0ID == abs((ir1->to())->particleID().pid()) &&
            (ir1)->to() == (ir2)->to()) {
          if (w > weightPart) {
            mcPart = (ir1)->to();
            weightPart = w;
