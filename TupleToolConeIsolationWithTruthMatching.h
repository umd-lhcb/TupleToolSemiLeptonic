#ifndef TUPLETOOLCONEISOLATIONWITHTRUTHMATCHING_H
#define TUPLETOOLCONEISOLATIONWITHTRUTHMATCHING_H 1
...
/** @class TupleToolConeIsolationWithTruthMatching
 * TupleToolConeIsolationWithTruthMatching.h
 *
 * \brief Fill track isolation for DecayTreeTuple.
 *    Open an eta-phi cone around the head, exclude all tracks that are in the
 * decay descriptor (i.e. that belong to the decay you are looking for), and
 * build variables with the remaining tracks
 *
 * Options:
 *
 ...
 * - ExtraParticlesLocation: set the type of particles that are considered in
 * the charged cone (default = "Phys/StdAllNoPIDsMuons/Particles")
 * - TrackType: set the type of tracks that are considered in the charged cone
 * (default = 3)
 * - FillNeutral: flag to fill the neutral cone (default = true)
 * - ExtraPhotonsLocation: set the type of particles that are considered in the
 ...
 * - FillMaxPt: flag to fill the momentum of the max-pT object in the cone
 * (default = true)
 * - MaxPtParticleLocation: set the type of max-pT particles that are considered
 * in the charged cone (default = "Phys/StdAllLooseMuons/Particles")
 * - FillComponents: flag to fill the components of all variables (default =
 * false)
 ...
 * - PizerosLocation: set the resolved pi0 type considered for combination with
 * the seed (default = Phys/StdLoosePi02gg/Particles)
 * - MergedPizerosLocation: set the merged pi0 type considered for combination
 * with the seed (default = Phys/StdLooseMergedPi0/Particles)
 ...
 *
 * - head_XX_asy_P : momentum asymmetry between the head and the cone defined as
 * (head_P - head_XX_P) / (head_P + head_XX_P)
 ...
 *
 * - head_XX_IT : transverse isolation of the head in the cone, defined as
 * head_PT / (head_P + head_XX_P)_T
 * - head_IT : transverse isolation of the head in the charged and neutral
 * cones, defined as head_PT / (head_P + head_cc_P + head_nc_P)_T
 *
 * - head_cc_maxPt_Q : charge of the max-pT object in the charged cone
 * - head_XX_maxPt_P : x, y, z (and e) components of the max-pT object momentum
 * in the cone
 ...
 * - head_Pi0Ph1_CL, head_Pi0Ph2_CL: confidence levels of the (photon) pi0
 * daughters
 * - For the merged pi0, the names of the variables are labelled replacing Pi0
 * by MergPi0
 *
 * \sa DecayTreeTuple, TupleToolTrackIsolationWithTruthMatching
 *
 *  @author Simone Bifani, Michel De Cian, Adlene Hicheur, Julian Garcia
 * Pardinas
 *  @date   2017-01-20
 *
 */
...
class TupleToolConeIsolationWithTruthMatching
 ...
 public:
  ...
  virtual ~TupleToolConeIsolationWithTruthMatching();  ///< Destructor
 ...
 private:
  ...
  StatusCode ChargedCone(const LHCb::Particle *seed,
                         ...
                         const LHCb::Particles *maxPts, const double rcut,
                         int &multiplicity, double &scalarPt,
                         std::vector<double> &momentum,
                         std::vector<double> &maxPt);

  /// Calculate properties of the neutral cone
  StatusCode NeutralCone(const LHCb::Particle *seed,
                         const LHCb::Particles *photons, const double rcut,
                         int &multiplicity, double &scalarPt,
                         std::vector<double> &momentum,
                         std::vector<double> &maxPt, double &AvConfLevel,
                         double &MaxConfLevel, double &LR1, double &LR2,
                         double &LR3, double &LR4, int &ConeTruthMatching);

  /// Check if a track belongs to the decay
  bool isTrackInDecay(const LHCb::Track *track);

  // fill pi0-track mass info
  StatusCode pi0Comb(const LHCb::Particle *seed, const LHCb::Particles *pi0s,
                     int &multiplicity, std::vector<double> &InvMass,
                     std::vector<double> &Pi0PX, std::vector<double> &Pi0PY,
                     std::vector<double> &Pi0PZ, std::vector<double> &Pi0E,
                     std::vector<double> &Pi0M, std::vector<double> &Pi0DeltaR,
                     std::vector<double> &Pi0Photon1CL,
                     std::vector<double> &Pi0Photon2CL,
                     std::vector<int> &Pi0TruthMatching);
};

#endif  // TUPLETOOLCONEISOLATIONWITHTRUTHMATCHING_H
