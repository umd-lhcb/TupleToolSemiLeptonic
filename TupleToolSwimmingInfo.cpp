...
bool TupleToolSwimmingInfo::hasDescendant(const LHCb::Particle* mother,
                                          const LHCb::Particle* desc) {
  if (!mother || !desc) {
    error() << "TupleToolSwimmingInfo::hasDescendant(" << mother << ", " << desc
            << ")" << endmsg;
    return false;
  }

  LHCb::Particle::ConstVector daughters(mother->daughtersVector());
  for (LHCb::Particle::ConstVector::iterator diter = daughters.begin();
       diter != daughters.end(); diter++) {
...
bool TupleToolSwimmingInfo::getRelationsFromDescendant(
    P2TPRelation::Range& range, const P2TPRelation* relatePart,
    const LHCb::Particle* Phead, const LHCb::Particle* P) {
  if (!relatePart || !Phead || !P) {
    error() << "TupleToolSwimmingInfo::getRelationsFromDescendant(range, "
            << relatePart << ", " << Phead << ", " << P << ")" << endmsg;
    return false;
  }
  ...
  for (LHCb::Particle::ConstVector::iterator diter = daughters.begin();
    ...
    if (range.size() == 1) {
      ...
      if (hasDescendant(daug, P)) {
        ...
        debug() << "getRelationsFromDescendant() found the correct B/D from "
                << relatePart->relations(P).size() << " choices" << endmsg;
        debug() << "swum descendant is " << *daug << " which has " << *P
                << " as a descendant" << endmsg;
        return true;
      } else {
        debug() << "Found particle " << daug
                << " with exactly one related swimming report, but it didn't "
                   "have our track as a descentant"
                << endmsg;
      ...
      if (getRelationsFromDescendant(range, relatePart, daug, P)) {
        debug() << "Succeeded calling getRelationsFromDescendant(range, "
                   "relatePart, "
                << daug << ", " << P << ")" << endmsg;
        return true;
      }
...
//=============================================================================
StatusCode TupleToolSwimmingInfo::fill(const Particle* Phead, const Particle* P,
  ...
  if (exist<P2TPRelation>(m_swimRelTableLoc)) {
    ...
    debug() << "Found Particle2TurningPointRelations at " << m_swimRelTableLoc
            << endmsg;
    if (!relatePart)
      return Error("Retrieved NULL Particle2TurningPointRelations");
  } else {
    error() << "Couldn't get Particle2TurningPointRelations from "
            << m_swimRelTableLoc << endmsg;
    return StatusCode::FAILURE;
  }

  // We need to find some reports. Any reports. Just so we can get a list of
  // names, so we always consistently set some value for all of the branches.
  // It's not (/no longer) acceptable to just return some error code without
  // tidying up when we find a candidate which wasn't swum
  stringSet decisions, infonames, extranames;
  P2TPRelation::Range allreports(relatePart->relations());
  for (P2TPRelation::Range::const_iterator report_iter = allreports.begin();
       report_iter != allreports.end(); report_iter++) {
    ...
    BOOST_FOREACH (const LHCb::TurningPoint& tp, turns) {
      BOOST_FOREACH (const std::string& name, tp.decisions()) {
        decisions.insert(
            name);  // things like Hlt2CharmHadD02HHXDst_hhXDecision,
                    // /Event/Phys/StripBlah/Particles
      }
  ...
  // Now we should have the information to know what all of our branches are
  // called
  debug() << "Found decisions:";
  for (stringSet::const_iterator dec_iter = decisions.begin();
       dec_iter != decisions.end(); dec_iter++)
    debug() << " " << *dec_iter;
  debug() << endmsg;
  debug() << "Found infonames:";
  for (stringSet::const_iterator inf_iter = infonames.begin();
       inf_iter != infonames.end(); inf_iter++)
    debug() << " " << *inf_iter;
  debug() << endmsg;
  debug() << "Found extranames:";
  for (stringSet::const_iterator xtra_iter = extranames.begin();
       xtra_iter != extranames.end(); xtra_iter++)
    debug() << " " << *xtra_iter;
  debug() << endmsg;
  ...
  if (P) {
    ...
    debug() << "Got range.size() = " << range.size() << endmsg;
    // range.size() will be zero if we failed to find any associated TPs -- in
    // this case we don't fill anything and nTP = 0. range.size() will be one if
    // everything is simple and there is only one swum candidate 'P' could
    // possibly be associated with range.size() will be larger if there are
    // multiple swum candidates and 'P' is part of more than one of them.
    LHCb::SwimmingReport* report(range.size() == 1 ? range.begin()->to()
                                                   : NULL);
    if (range.size() == 1 and !report)
      return Error("Got NULL LHCb::SwimmingReport");

    if (range.size() > 1) {
      // There can be multiple B/D candidates swum in one event. This means
      // there will be multiple SwimmingReports. A particular B/D descendant
      // track/particle may be part of more than one B/D candidate, and
      // therefore we may find more than one related SwimmingReport in the
      // relations table. In this case we need to find which SwimmingReport
      // corresponds to the B/D candidate which we are currently processing.
      // Start searching from 'Phead' downwards, until we find a particle with
      // exactly one SwimmingReport and which has 'P' as a descendant.
      debug() << "Phead = " << Phead << endmsg;
      if (Phead) debug() << "*Phead = " << *Phead << endmsg;
      if (getRelationsFromDescendant(range, relatePart, Phead, P) &&
          range.size() == 1) {
        debug() << "Successfully found relations despite range.size() "
                   "initially being > 1"
                << endmsg;
    ...
    if (!P->isBasicParticle()) {
      ...
      // If there is only one decision then the next part would just be
      // duplicating ..._TP_DEC above.
      bool doDecisions(decisions.size() > 1);

      // This is just setting up some structures rather than actually filling
      // turning point values Even if we didn't swim this event, we still need
      // to fill in the right branches in the tree This is why 'decisions' was
      // populated earlier...
      for (stringSet::const_iterator dec_iter = decisions.begin();
           doDecisions && dec_iter != decisions.end(); dec_iter++)
      ...
      if (report) {
        // After all that, we do have swimming reports to associate with this
        // event
        const tPoints& turns = report->turningPoints(m_swimRepsStage);
      ...
      if (raw.size() > maxTurns)
        warning() << "Have " << raw.size()
                  << " turning points, which is > maxTurns = " << maxTurns
                  << endmsg;
      ...
      BOOST_FOREACH (const MapType::value_type& entry, extra_info) {
        test &= tuple->farray(prefix + "_TP_" + entry.first, entry.second,
                              prefix + "_nTP", maxTurns);
      }
    } else if (m_swimRepsStage.size() >= triggerPrefix.size() and
               m_swimRepsStage.substr(0, triggerPrefix.size()) ==
                   triggerPrefix) {
      ...
      for (stringSet::const_iterator infoname_iter = infonames.begin();
           infoname_iter != infonames.end(); infoname_iter++)
