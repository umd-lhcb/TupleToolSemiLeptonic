...
StatusCode TupleToolTauMuDiscrVars::fill(const Particle* mother,
  ...
  if (mother && P) {
    ...
    const double Dp_M = 1869.59;
    const LoKi::LorentzVector D_PXYZE_unconstrained =
        P->daughters()[0]->momentum();
    const LoKi::LorentzVector D_PXYZE(
        D_PXYZE_unconstrained.Px(), D_PXYZE_unconstrained.Py(),
        D_PXYZE_unconstrained.Pz(),
        ::sqrt(D_PXYZE_unconstrained.P2() + Dp_M * Dp_M));
