...
StatusCode TupleToolSubMass::initialize() {
  ...
  // consistentcy checks
  for (std::vector<std::string>::const_iterator is = m_subst.begin();
       m_subst.end() != is; ++is) {
    ...
    if (NULL == property(from))
      return Warning("Cannot substitute from unknown particle " + from,
                     StatusCode::FAILURE);
    if (NULL == property(to))
      return Warning("Cannot substitute to unknown particle " + to,
                     StatusCode::FAILURE);
  }
  for (std::vector<std::string>::const_iterator is = m_subst2.begin();
       m_subst2.end() != is; ++is) {
    const std::pair<std::string, std::string> fromto = parseSubst(*is);
    const std::string from = fromto.first;
    const std::string to = fromto.second;
    if (parseID(from).first == "NONE")
      return Warning("Cannot substitute from unknown particles pair " + from,
                     StatusCode::FAILURE);
    if (parseID(to).first == "NONE")
      return Warning("Cannot substitute to unknown particles pair " + from,
                     StatusCode::FAILURE);
  }
  return sc;
}
...
StatusCode TupleToolSubMass::fill(const LHCb::Particle*,
  ...
  for (unsigned int pos = 0; pos < tree.size(); pos++) {
    if (pos >= tree.size())
      return Warning("Unexpected error", StatusCode::FAILURE);
    // if(msgLevel(MSG::DEBUG))debug() << " Ancestor : " <<
    // P->particleID().pid() << " daughter " << pos << " : " <<
  ...
  if (msgLevel(MSG::DEBUG))
    debug() << "===> sorted decay structure for " << P->particleID().pid()
            << " : " << decay << endmsg;
  ...
  for (unsigned int level = 2; level <= max; ++level) {
    for (unsigned int d = 0; d < max - level + 1; ++d) {
      ...
      for (std::vector<std::vector<int> >::const_iterator ivv = out.begin();
           ivv != out.end(); ++ivv) {
        ...
        if (fMass.first != "NONE")
          fill &= tuple->column(prefix + "_M" + fMass.first, fMass.second);
        ...
        for (std::vector<std::string>::const_iterator is = m_subst.begin();
          ...
          for (std::vector<int>::const_iterator im = map.begin();
               map.end() != im; ++im) {
            int pos = *im;
            const LHCb::Particle* part = tree[pos];
            if (part->particleID().abspid() == fromID) {
              std::pair<std::string, double> sMass =
                  getMass(tree, map, pos, toMass);
              if (sMass.first != "NONE")
                fill &= tuple->column(
                    prefix + "_M" + sMass.first + "_" + getFlag(from, to, pos),
                    sMass.second);
            }
          }
        }
        //==== Try double substitutions when requested
        for (std::vector<std::string>::const_iterator iss = m_subst2.begin();
             m_subst2.end() != iss; ++iss) {
          ...
          for (unsigned int i1 = 0; i1 < size; i1++) {
            for (unsigned int i2 = i1 + 1; i2 < size; i2++) {
              ...
              if ((pid1 == fromID1 && pid2 == fromID2) ||
                  (apid1 == fromID1 && apid2 == fromID2)) {
                std::pair<std::string, double> sMass =
                    getMass(tree, map, pos1, toMass1, pos2, toMass2);
                if (sMass.first != "NONE") {
                  fill &= tuple->column(
                      prefix + "_M" + sMass.first + "_" +
                          getFlag(from1, to1, pos1, from2, to2, pos2),
                      sMass.second);
                }
              } else if ((pid1 == fromID2 && pid2 == fromID1) ||
                         (apid2 == fromID1 && apid1 == fromID2)) {
                std::pair<std::string, double> sMass =
                    getMass(tree, map, pos1, toMass2, pos2, toMass1);
                if (sMass.first != "NONE") {
                  fill &= tuple->column(
                      prefix + "_M" + sMass.first + "_" +
                          getFlag(from1, to1, pos1, from2, to2, pos2),
...
std::pair<std::string, std::string> TupleToolSubMass::parseSubst(
    std::string subst) {
...
std::string TupleToolSubMass::getFlag(std::string from, std::string to, int pos,
                                      std::string from2, std::string to2,
                                      int pos2) {
  ...
  boost::replace_all(to2, "-", "");
  if (pos2 < 0)
    return "Subst" + Gaudi::Utils::toString(pos) + "_" + from + "2" + to;
  return "Subst" + Gaudi::Utils::toString(pos) + Gaudi::Utils::toString(pos2) +
         "_" + from + from2 + "2" + to + to2;
}

//------- compute momentum (possibly with PID substitution)
Gaudi::LorentzVector TupleToolSubMass::sMomentum(const LHCb::Particle* part,
                                                 double sMass) {
  Gaudi::LorentzVector momentum = part->momentum();
  if (isPureNeutralCalo(part)) {  // photon (E is measured) - use-case :  gamma
                                  // <->pi0 substitution
    if (sMass < momentum.E()) {
...
bool TupleToolSubMass::fillVertexInfo(const LHCb::Particle* P,
  ...
  std::string flag;
  for (std::vector<int>::const_iterator iv = map.begin(); iv != map.end();
       ++iv) {
    int pos = *iv;
    const LHCb::Particle* part = tree[pos];
...
std::pair<std::string, double> TupleToolSubMass::getMass(
    std::vector<const LHCb::Particle*> tree, const std::vector<int> map,
  ...
  if ((!doSub1 && sPos >= 0) || (!doSub2 && sPos2 >= 0))
...
bool TupleToolSubMass::isEndTree(const LHCb::Particle* p) {
  ...
  for (std::vector<int>::iterator it = m_endTree.begin(); m_endTree.end() != it;
       ++it) {
...
std::vector<const LHCb::Particle*> TupleToolSubMass::getTree(
    const LHCb::Particle* P) {
  std::vector<const LHCb::Particle*> tree;
  ...
  if (isEndTree(P)) {
    ...
    SmartRefVector<LHCb::Particle> daughters =
        P->daughters();  // local copy to sort
    std::stable_sort(daughters.begin(), daughters.end(),
                     SortDaughtersByPID(m_ancestor, m_ppsvc));
    for (SmartRefVector<LHCb::Particle>::const_iterator idau =
             daughters.begin();
         idau != daughters.end(); ++idau) {
      std::vector<const LHCb::Particle*> temp = getTree(*idau);
      for (std::vector<const LHCb::Particle*>::iterator dd = temp.begin();
           temp.end() != dd; ++dd) {
        tree.push_back(*dd);
      }
    }
...
std::vector<std::vector<int> > TupleToolSubMass::combine(
    std::vector<std::vector<int> > vec, unsigned int level, int nfs) {
  if ((vec.back()).size() == level) return vec;
  std::vector<std::vector<int> > cc;  // local vector
  for (std::vector<std::vector<int> >::iterator iv = vec.begin();
       iv != vec.end(); ++iv) {
