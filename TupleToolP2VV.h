 ...
 * - head_Theta_phi_tr : polar angle between the x'-axis and the decaying K+ in
 * the dikaon frame
 *
...
class TupleToolP2VV : public TupleToolBase, virtual public IParticleTupleTool {
 public:
  ...
  TupleToolP2VV(const std::string& type, const std::string& name,
                const IInterface* parent);

  ~TupleToolP2VV();  ///< Destructor
 ...
 private:
  std::string m_calculator;  ///< Name of tool that you want to use to calculate
                             ///< the angles
