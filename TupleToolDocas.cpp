...
TupleToolDocas::TupleToolDocas(const std::string& type, const std::string& name,
...
StatusCode TupleToolDocas::initialize() {
  ...
  for (auto f = m_name.begin(); f != m_name.end(); ++f) {
    ...
    if (m_name.end() != ff) {
      return Error("Two identical branch names in Name", StatusCode::FAILURE);
    }
  }
...
StatusCode TupleToolDocas::fill(const Particle*, const Particle* P,
  ...
  for (size_t i = 0; i < m_name.size(); ++i) {
    ...
    if (nullptr == daughter1 || nullptr == daughter2) {
      Error("didn't find daughters for location", 20, StatusCode::SUCCESS)
          .ignore();
      if (nullptr == daughter1)
        Error(std::string(" failed with location 1: ") + m_locations1[i], 20,
              StatusCode::SUCCESS)
            .ignore();
      if (nullptr == daughter2)
        Error(std::string(" failed with location 2: ") + m_locations2[i], 20,
              StatusCode::SUCCESS)
            .ignore();
    ...
    Info(std::string("created column ") + prefix + nam, 1, StatusCode::SUCCESS)
        .ignore();
    Info(std::string("created column ") + prefix2 + nam, 1, StatusCode::SUCCESS)
        .ignore();
  }
