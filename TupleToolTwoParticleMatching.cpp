...
//=============================================================================
TupleToolTwoParticleMatching::TupleToolTwoParticleMatching(
...
StatusCode TupleToolTwoParticleMatching::initialize() {
  ...
  for (std::vector<std::string>::iterator iName = m_toolNames.begin();
       iName != m_toolNames.end(); ++iName) {
    if ((*iName) == "TupleToolGeometry") {
      warning() << "Geometry tuple tool can not be used in matching (yet). "
                   "Please find a way how to include it by yourself."
                << endmsg;
    } else {
      m_tuple.push_back(
          tool<IParticleTupleTool>(*iName, *iName + ":PUBLIC", this));
    }
  }

  if (m_matching_location != "OBSOLETE") {
    error() << "Option MatchWith is deprecated. Use MatchLocations instead."
            << endmsg;
  ...
  for (matchLocation = m_matching_locations.begin();
       matchLocation != m_matching_locations.end(); matchLocation++) {
...
StatusCode TupleToolTwoParticleMatching::fill(const LHCb::Particle*,
  ...
  if (m_parsed_locations.count(apid)) {
    ...
    sc = sc && tuple->column("NMatchingCand_" + fullName(head) + m_headSuffix,
                             counter);
    sc = sc &&
         tuple->column("Overlap_loaded_" + fullName(head) + m_headSuffix, o1);
    sc = sc &&
         tuple->column("Overlap_original_" + fullName(head) + m_headSuffix, o2);

    for (std::vector<IParticleTupleTool*>::const_iterator iTool =
             m_tuple.begin();
         iTool != m_tuple.end(); ++iTool)
