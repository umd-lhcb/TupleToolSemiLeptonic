...
TupleToolMassHypo::TupleToolMassHypo(const std::string& type,
    ...
    : TupleToolBase(type, name, parent), m_first(true), m_transporter() {
  declareInterface<IParticleTupleTool>(this);
  declareProperty("PIDReplacements", m_replace,
                  "List of PID replacements as dictionary of strings");
...
StatusCode TupleToolMassHypo::initialize() {
  ...
  for (std::map<std::string, std::string>::const_iterator iterS =
           m_replace.begin();
    ...
    if (!prop1 || !prop2) {
      err() << "one of " << iterS->first << " " << iterS->second
            << " does nor exist" << endmsg;
    ...
    m_massMap.insert(
        std::pair<int, double>(prop2->pdgID().pid(), prop2->mass()));
    m_nameMap.insert(
        std::pair<int, std::string>(pid, Decays::escape(prop1->name())));
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Inserted name ``" << Decays::escape(prop1->name()) << "''"
                << endmsg;
    m_nameMap.insert(std::pair<int, std::string>(
        prop2->pdgID().pid(), Decays::escape(prop2->name())));
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Inserted name ``" << Decays::escape(prop2->name()) << "''"
                << endmsg;
    m_replacePDG.insert(std::pair<int, int>(pid, prop2->pdgID().pid()));
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Inserted PID pair ``" << pid << "," << prop2->pdgID().pid()
                << "''" << endmsg;
  }  // iterS
  m_transporter =
      tool<IParticleTransporter>("ParticleTransporter:PUBLIC", this);
...
StatusCode TupleToolMassHypo::fill(const LHCb::Particle*,
  ...
  if (msgLevel(MSG::VERBOSE))
    verbose() << "Entering with a " << P->particleID().pid() << endmsg;
  ...
  for (possDecayVec::const_iterator p = possibilities.begin();
       p != possibilities.end(); ++p) {
    std::string combname;
    if (msgLevel(MSG::VERBOSE))
    ...
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Filled column ``" << prefix + "_M_with" + combname
...
//=============================================================================
double TupleToolMassHypo::recalculateMass(const possDecay& pd,
                                          const Gaudi::XYZPoint& pt,
                                          std::string& combname) {
  Gaudi::LorentzVector newM;
  for (possDecay::const_iterator p = pd.begin(); p != pd.end(); ++p) {
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Loop ``" << combname << "'' pid : ``" << p->second << "''"
                << endmsg;
    const LHCb::Particle* P = p->first;
    if (m_nameMap.find(p->second) != m_nameMap.end())
    ...
    if (P->particleID().pid() != p->second ||
        (m_revert && P->particleID().pid() != -p->second)) {
    ...
    newM += mom;
    if (msgLevel(MSG::VERBOSE))
      verbose() << "Total Momentum is now " << newM << endmsg;
  }
  if (msgLevel(MSG::VERBOSE))
...
//=============================================================================
possDecayVec TupleToolMassHypo::getPossibilities(
    const LHCb::Particle::ConstVector& dv) {
  possDecayVec possibilities;
  // loop over daghters
  for (LHCb::Particle::ConstVector::const_iterator d1 = dv.begin();
       d1 != dv.end(); ++d1) {
    if (msgLevel(MSG::VERBOSE))
      verbose() << "getPossibilities Looping over " << (*d1)->particleID().pid()
                << endmsg;
    possibilities = increaseVector(*d1, possibilities);
    if (msgLevel(MSG::VERBOSE))
      verbose() << "getPossibilities Size of possibilities "
                << possibilities.size() << endmsg;
  }
  if ((possibilities.size() > 1) && (msgLevel(MSG::DEBUG) || m_first))
    printVec(possibilities);
  if (m_first) m_first = false;
  return possibilities;
}
//=============================================================================
void TupleToolMassHypo::addPossPair(const LHCb::Particle* d, int pid,
                                    possDecay& pm) {
  pm.push_back(possPair(d, pid));
  if (msgLevel(MSG::VERBOSE))
    verbose() << "addPossPair Added " << pid << " size: " << pm.size()
              << endmsg;
}

//=============================================================================
// multiplies the vector
possDecayVec TupleToolMassHypo::increaseVector(const LHCb::Particle* d,
                                               possDecayVec& poss) {
  int pid = d->particleID().pid();
  if (m_revert) pid = abs(pid);
  if (msgLevel(MSG::VERBOSE))
    verbose() << "increaseVector seeing a " << d->particleID().pid()
              << " treated as " << pid << endmsg;
  possDecayVec tmp;
  if (poss.empty()) {
    possDecay pm;
...
void TupleToolMassHypo::printVec(const possDecayVec& possibilities) {
  for (possDecayVec::const_iterator i = possibilities.begin();
