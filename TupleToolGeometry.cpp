...
//=============================================================================
StatusCode TupleToolGeometry::fill(const Particle* mother, const Particle* P,
  ...
  if (isPureNeutralCalo(P))
    return Warning(
        "Will not fill geometry tuple for neutral Calo particles. No worry",
        StatusCode::SUCCESS, 10);
  if (msgLevel(MSG::VERBOSE))
    verbose() << "TupleToolGeometry::fill " << mother << " " << P << " "
              << prefix << endmsg;
  ...
  if (P->isBasicParticle()) {
    ...
    //=========================================================================
    if (msgLevel(MSG::VERBOSE) &&
        P->endVertex()) {  // https://savannah.cern.ch/bugs/?92524
      verbose() << "Before cast : " << P->endVertex() << endmsg;
      verbose() << "Container "
                << P->endVertex()->parent()->registry()->identifier() << " key "
    ...
    if (!evtx) {
      fatal() << "Can't retrieve the end vertex for " << prefix
              << ". TupleToolGeometry cannot act on such a particle. Define a "
                 "branch"
              << endmsg;
      fatal() << "See "
                 "https://twiki.cern.ch/twiki/bin/view/LHCb/"
                 "DaVinciTutorial7#Branches"
              << endmsg;
  ...
  if (mother && mother != P) {
    ...
    if (isVerbose()) {
      ...
      if (!P->isBasicParticle())
        sc = fillFlight(aPV, P, prefix, tuple, "_ORIVX");
      if (sc.isFailure()) {
