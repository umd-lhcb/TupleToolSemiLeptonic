...
//=============================================================================
TupleToolPid::TupleToolPid(const std::string& type, const std::string& name,
...
StatusCode TupleToolPid::fill(const Particle*, const Particle* P,
  ...
  if (P) {
    ...
    if (!P->isBasicParticle())
      return StatusCode(test);  // no PID info for composite!
    if (isPureNeutralCalo(P))
      return StatusCode(test);  // no PID information for calo neutrals
    ...
    if (proto) {
      ...
      test &=
          tuple->column(prefix + "_ProbNNghost",
      ...
      test &= tuple->column(prefix + "_isMuon",
                            muonPID ? muonPID->IsMuon() : false);

      // RICH
      const RichPID* richPID = proto->richPID();
      test &= tuple->column(prefix + "_hasRich", richPID != NULL);
      // Moved some RICH information variables to default since they are useful
      // to determine how the PID information has been obtained.
      ...
      test &=
          tuple->column(prefix + "_RichAboveElThres",
      ...
      if (UNLIKELY(isVerbose())) {
        ...
        test &= tuple->column(prefix + "_isMuonLoose",
                              muonPID ? muonPID->IsMuonLoose() : false);
        test &= tuple->column(prefix + "_isMuonTight",
                              muonPID ? muonPID->IsMuonTight() : false);
        test &= tuple->column(prefix + "_MuonMuLL",
                              muonPID ? muonPID->MuonLLMu() : -1000);
        test &= tuple->column(prefix + "_MuonBkgLL",
                              muonPID ? muonPID->MuonLLBg() : -1000);
        test &= tuple->column(prefix + "_MuonNShared",
                              muonPID ? muonPID->nShared() : -1);
        ...
        test &=
            tuple->column(prefix + "_CaloEcalE",
                          proto->info(LHCb::ProtoParticle::CaloEcalE, -10000.));
        test &=
            tuple->column(prefix + "_EcalPIDe",
                          proto->info(LHCb::ProtoParticle::EcalPIDe, -10000.));
        test &=
            tuple->column(prefix + "_EcalPIDmu",
        ...
        test &=
            tuple->column(prefix + "_CaloHcalE",
                          proto->info(LHCb::ProtoParticle::CaloHcalE, -10000.));
        test &=
            tuple->column(prefix + "_HcalPIDe",
                          proto->info(LHCb::ProtoParticle::HcalPIDe, -10000.));
        test &=
            tuple->column(prefix + "_HcalPIDmu",
