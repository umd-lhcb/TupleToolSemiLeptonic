...
//
// 2012-03-17 : Fatima Soomro, based on IsolationTwoBody (in DaVinciTools) by
...
StatusCode TupleToolIsolationTwoBody::fill(const LHCb::Particle* /* top */,
  ...
  StatusCode sc;
  if (part->isBasicParticle() && m_MuChi2) {
    sc = MuChi2(part, prefix, tuple);
  }
  ...
  if (part->isBasicParticle() || part->daughtersVector().size() != 2) {
    ...
    for (Particle::ConstVector::const_iterator ip_part = parts.begin();
         ip_part != parts.end(); ip_part++) {
      if (!((*ip_part)->isBasicParticle())) shdBeTrue = false;
      if (!shdBeTrue && msgLevel(MSG::DEBUG))
        debug() << "Found compositle particle "
                << (*ip_part)->particleID().pid() << " in the daughters of"
...
StatusCode TupleToolIsolationTwoBody::MuChi2(const LHCb::Particle* P,
  ...
  if (msgLevel(MSG::DEBUG))
    debug() << "checking for muonpids at " << LHCb::MuonPIDLocation::Default
            << endmsg;
  ...
  if (!pMuids) {
    error() << " Failed to get pMuids" << LHCb::MuonPIDLocation::Default
            << endmsg;
  ...
  */
  ...
  for (ip = allparts->begin(); ip != allparts->end(); ++ip) {
    ...
    if (ratio(pt, ptmu[0]) < 0.0001 || ratio(pt, ptmu[1]) < 0.0001) {
      isInList = 1;
      if (msgLevel(MSG::DEBUG))
        debug() << "isInList is found to be " << isInList << endmsg;
    }
    ...
    if (!isInList) {
      ...
      for (i2 = 0; i2 < 2; i2++) {
        ...
        // if (m_isMC) {
        //  if (i2 == 0 && mclink && mclink1) isolating =
        //  ancestor(mclink)==ancestor(mclink1); if (i2 == 1 && mclink &&
        //  mclink2) isolating = ancestor(mclink)==ancestor(mclink2);
        //}
        ...
        pvdis = (vtx.z() - PosPV.z()) / fabs(vtx.z() - PosPV.z()) *
                (vtx - PosPV).R();
        svdis = (vtx.z() - PosSV.z()) / fabs(vtx.z() - PosSV.z()) *
                (vtx - PosSV).R();
        ...
        if (angle < m_angle && fc < m_fc && doca < m_doca && ips > m_ips &&
            svdis > m_svdis && svdis < m_svdis_h && pvdis > m_pvdis &&
            pvdis < m_pvdis_h && clone > m_clone && ghost < m_ghost &&
            track->type() == m_tracktype && trchi2 < m_trchi2 &&
            hltgood > m_hltgood) {
          // is_iso5 = true;
  ...
  if (iso_parts_0.size() > 1) {
    ...
    if (msgLevel(MSG::DEBUG))
      debug() << "trying to get to this stage " << endmsg;
  ...
  //    fatima, fill here
  if (msgLevel(MSG::DEBUG)) {
    debug() << "massiso " << massiso[0] << "  " << massiso[1] << " chi2iso "
            << chi2iso[0] << "  " << chi2iso[1] << endmsg;
...
void TupleToolIsolationTwoBody::IsHltGood(Gaudi::XYZPoint o, Gaudi::XYZVector p,
  ...
  if (fail) {
    ...
    if ((DOCA_b < 0.2) && (ZZ > 0.) && (ZZ < 30.) && (fc < 0.4) && (pete > 2.))
      hltgood = 1;
  }
}
...
//
//=============================================================================
void TupleToolIsolationTwoBody::InCone(Gaudi::XYZPoint o1, Gaudi::XYZVector p1,
                                       Gaudi::XYZPoint o2, Gaudi::XYZVector p2,
                                       Gaudi::XYZPoint& vtx, double& doca,
                                       double& angle) const {
  Gaudi::XYZPoint rv;
  Gaudi::XYZPoint close;
  Gaudi::XYZPoint close_mu;
...
//
//=============================================================================
void TupleToolIsolationTwoBody::closest_point(
    Gaudi::XYZPoint o, Gaudi::XYZVector p, Gaudi::XYZPoint o_mu,
...
//
//=============================================================================
const LHCb::MCParticle* TupleToolIsolationTwoBody::ancestor(
    const LHCb::MCParticle* imc) const {
  if (imc->mother()) {
