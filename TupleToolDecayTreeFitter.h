 ...
 * -  History : this tool was written during the study of the b->JpsiX
 lifetimes. It was not very generic.
 *    This re-implmentation is hopefully more generic, although it requires
 several instances of the tool
 *    to achive the same result.
 * -  Advice  : Most of the output of this code could be saved via the LoKi
 style lines.

 * -  Usage   : One can plug this Tool in DecayTreeTuple. It is advised to give
 it to a branch and not to try to run
 ...
 *  - Configuration :
 ...
 particlesToConstrain = ["J/psi(1S)"]     # chose the particles to mass to
 constrain. ExtraName = ""                           # Additional Prefix (will
 replace instance name) (inherited from TupleToolBase) Verbose = False # Fill
 Lifetime for daughters. (inherited from TupleToolBase)
 @endcode
 *
 ...
 * -  Example:
 ...
 tuple.ToolList +=  ["TupleToolGeometry",  "TupleToolKinematic",
 "TupleToolPrimaries", "TupleToolEventInfo" ] tuple.Branches = { "B"  :
 "["+bh+"]cc : "+decay.replace("^","") } from Configurables import
 TupleToolDecayTreeFitter tuple.B.ToolList +=  [ "TupleToolDecayTreeFitter/Fit",
 # just a refit "TupleToolDecayTreeFitter/MassFit",        # fit with J/psi mass
 constraint "TupleToolDecayTreeFitter/PVFit" ]         # fit with all
 ...
 tuple.B.ToolList +=  ["TupleToolDecayTreeFitter/SubLambda",
 "TupleToolDecayTreeFitter/SubLambdabar" ] subDTF =
 TupleToolDecayTreeFitter("SubLambda", Verbose=True, daughtersToConstrain = [
 "J/psi(1S)" ], constrainToOriginVertex=True, Substitutions={ 'Beauty -> Meson
 (Strange -> ^pi+ pi-)': 'p+' }) tuple.B.addTool(subDTF)
 tuple.B.addTool(subDTF.clone("SubLambdabar",Substitutions={ 'Beauty -> Meson
 (Strange -> pi+ ^pi-)': 'p-' }))

 @endcode
 *
 * - This will produce the following columns for the B (from this tool):
 *     - B_Fit_status B_Fit_nDOF B_Fit_chi2_B B_Fit_nIter B_Fit_M B_Fit_MERR
 B_Fit_P B_Fit_PERR
 *     - B_MassFit_status B_MassFit_nDOF B_MassFit_chi2_B B_MassFit_nIter
 B_MassFit_M B_MassFit_MERR B_MassFit_P
 *       B_MassFit_PERR B_PVFit_status B_PVFit_nDOF B_PVFit_chi2_B B_PVFit_nIter
 B_PVFit_M B_PVFit_MERR B_PVFit_P
 *     - B_PVFit_PERR B_PVFit_ctau B_PVFit_ctauErr B_PVFit_decayLength
 B_PVFit_decayLengthErr B_PVFit_J_psi_1S_ctau
 *       B_PVFit_J_psi_1S_ctauErr B_PVFit_J_psi_1S_decayLength
 B_PVFit_J_psi_1S_decayLengthErr B_PVFit_KS0_ctau
 *       B_PVFit_KS0_ctauErr B_PVFit_KS0_decayLength B_PVFit_KS0_decayLengthErr
 *
 * - One can also hange the PID of some particles in the chain, using the same
 syntax as SubstitutePID
 *
 * \sa SubstitutePID
 *
 * \sa DecayTreeTuple
 *
 *  @author Yasmine Amhis, Matt Needham (original authors), Patrick Koppenburg
 (re-implementation)
 ...
 * 30-10-10
 */
class TupleToolDecayTreeFitter : public TupleToolBase,
                                 virtual public IParticleTupleTool {
 private:
  typedef std::map<std::string, std::string> SubstitutionMap;
  typedef std::map<std::string, std::vector<double> >
      TupleMap;  ///< temporary storage of tuple data

 public:
  /// Standard constructor
  TupleToolDecayTreeFitter(const std::string& type, const std::string& name,
                           const IInterface* parent);

  ~TupleToolDecayTreeFitter(){};  ///< Destructor
 ...
 private:
  ...
  StatusCode fit(DecayTreeFitter::Fitter& fitter, const LHCb::Particle* P,
  ...
  ///  Fill PV information
  StatusCode fillPV(const LHCb::VertexBase* pv, const std::string& prefix,
                    TupleMap& tMap) const;
  ...
  StatusCode fillTuple(const TupleMap& tMap, Tuples::Tuple& tuple,
                       const std::string& prefix) const;
  /// insert helper method
  StatusCode insert(const std::string& leaf, const double val,
                    TupleMap& tMap) const;

  std::string getName(const int id) const;  ///< name of particle

  ///  origin vertex
  std::vector<const LHCb::VertexBase*> originVertex(
 ...
 private:
  std::string m_pvLocation;  ///<  PV location to be used. If empty, take
  ...
  ISubstitutePID::SubstitutionMap
      m_map;  // mapping : { 'decay-component' : "new-pid" }
  /// Substitute Tool
  ISubstitutePID* m_substitute;  // tool
  bool m_storeAnyway;  ///< Store PV even if a refitted version is already the
                       ///< best PV (i.e store twice)
  bool
      m_updateDaughters;  ///< Store updated momenta of tracks in the decay tree
