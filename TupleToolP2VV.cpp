...
//=============================================================================
TupleToolP2VV::TupleToolP2VV(const std::string& type, const std::string& name,
...
//  Fill
//=========================================================================
StatusCode TupleToolP2VV::fill(const LHCb::Particle*, const LHCb::Particle* P,
  ...
  if (P) {
    if (m_helicity) {
      ...
      if (msgLevel(MSG::DEBUG))
        debug() << "Three helicity angles are theta_L : " << thetaL
                << " K: " << thetaK << " phi: " << phi << endmsg;
    ...
    if (m_trans) {
      ...
      StatusCode sc_tr = m_angleTool->calculateTransversityAngles(
          P, Theta_tr, Phi_tr, Theta_V);

      if (msgLevel(MSG::DEBUG))
        debug() << "Three transversity angles are Theta_tr : " << Theta_tr
                << " Phi_tr: " << Phi_tr << " Theta_phi_tr: " << Theta_V
                << endmsg;
