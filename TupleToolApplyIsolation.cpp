...
StatusCode TupleToolApplyIsolation::fill(const Particle* mother, const Particle* P, const std::string& head, Tuples::Tuple& tuple)
{
    ...
    for (std::vector<std::string>::iterator i = m_inputParticles.begin();
        ...
        for (LHCb::Particle::Range::const_iterator iparts = (parts).begin();
            ...
            if (part->proto()->track()->type() < 5 && !isTrackInDecay(part->proto()->track(), daughtertracks)) {
                ...
                //warning() << "type " << type << " opening " << opening << " pt " << pt << endmsg;
                ...
                if (chi2 < 50) {
                    ...
                    //warning() << "bdtval " << bdtval << " old maxbdt " << maxbdt << endmsg;	        if (bdtval > maxbdt) {
                    bdt3 = bdt2;
                    bdt2 = maxbdt;
                    maxbdt = bdtval;
                    part3 = part2;
...
//
//=========================================================================
const Vertex* TupleToolApplyIsolation::originVertex(const Particle* top, const Particle* P) const
{
