...
Tuple.TupleToolSubMass.SetMax = 3  # look to 2- and 3-bodies  (defaut all
possible n-bodies) \endverbatim
 *
 * - will create Sum_{p=2,max) C(N,p)  new entries in the nTuple called
[head]_M[ij...]
 *
 * - Particle daughters are sorted by PID at each branch of the tree
(cc-independant)
 *
 ...
 *
 * -change only one pion (K) at once in case of several pion (K) in the decay
tree (producing separate output par pion (K) )
 *
 ...
 * - change only one pair at once in case of several pairs in the decay tree
(producing separate output per pair)
 * - "/" separator is not mandatory : K+pi- syntax is allowed (just a bit slower
to parse)
 *
...
class SortDaughtersByPID {
 public:
  ...
  bool operator()(const LHCb::Particle* c1, const LHCb::Particle* c2) {
    int p1 = (property(c1->particleID())->selfcc())
                 ? c1->particleID().pid()
                 : m_sign * c1->particleID().pid();
    int p2 = (property(c2->particleID())->selfcc())
                 ? c2->particleID().pid()
                 : m_sign * c2->particleID().pid();
    // special case  c1 == c2 but with different decay structure (e.g.
    // pi0->g(g->ee) : check daughters size
    if (p1 == p2 && c1->daughters().size() != c2->daughters().size())
      return c1->daughters().size() > c2->daughters().size();
    // add a complete test of the decay structure (above test won't work for
    // e.g. Bs->phi(KsKs)phi(K+K-) decay
    // ..
    // else
    return p1 > p2;
  };
...
class TupleToolSubMass : public TupleToolBase,
                         virtual public IParticleTupleTool {
 public:
  /// Standard constructor
  TupleToolSubMass(const std::string& type, const std::string& name,
 ...
 private:
  std::vector<std::vector<int> > combine(std::vector<std::vector<int> > vec,
                                         unsigned int level, int nfs);
  std::vector<const LHCb::Particle*> getTree(const LHCb::Particle* P);
  bool isEndTree(const LHCb::Particle* p);
  bool fillVertexInfo(const LHCb::Particle* P,
                      std::vector<const LHCb::Particle*> tree,
                      const std::vector<int> map, Tuples::Tuple& tuple,
                      const std::string prefix);
  std::pair<std::string, double> getMass(
  ...
  LHCb::IParticlePropertySvc* m_ppsvc;
  const LHCb::ParticleProperty* property(std::string name) {
    return (m_ppsvc) ? m_ppsvc->find(name) : NULL;
  };
  const LHCb::ParticleProperty* property(const LHCb::ParticleID pid) {
    return (m_ppsvc) ? m_ppsvc->find(pid) : NULL;
  };
