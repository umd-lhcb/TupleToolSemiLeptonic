...
StatusCode TupleToolParticleStats::fill(Tuples::Tuple& tup) {
  ...
  for (std::vector<std::string>::const_iterator l = m_locations.begin();
    ...
    if (!(exist<LHCb::Particles>("/Event/Phys/" + *l + "/Particles"))) {
      error()
          << " your configuration is problematic, tthe location you requested: "
          << endmsg;
      error() << " >>>  " << ("/Event/Phys/" + *l + "/Particles")
              << " <<<  doesnt seem to have been filled." << endmsg;
      continue;
    }
    test &= tup->column(
        prefix + "NumberOf" + *l,
        number<LHCb::Particles>("/Event/Phys/" + *l + "/Particles"));
  }
