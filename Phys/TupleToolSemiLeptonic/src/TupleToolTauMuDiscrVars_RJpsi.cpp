// local
#include "TupleToolTauMuDiscrVars_RJpsi.h"
...
DECLARE_COMPONENT( TupleToolTauMuDiscrVars_RJpsi )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
TupleToolTauMuDiscrVars_RJpsi::TupleToolTauMuDiscrVars_RJpsi( const std::string& type,
  ...
  declareProperty( "PVReFitter", m_pvReFitterName,
                   "PV refitter algorithm name (':PUBLIC' at end of algo name "
...
//=============================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::initialize() {
...
//=============================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fill( const Particle*    mother,
                                          const Particle*    P,
  ...
  if ( mother && P ) {
    ...
    // Bc estimated momentum.
    const double B_M           = 6274.47;
    const double observed_B_M  = P->measuredMass();
    const double observed_B_PZ = P->momentum().Z();
    const double B_P           = B_M / observed_B_M * observed_B_PZ *
                       ::sqrt( 1. + ::tan( B_angle_Z ) * ::tan( B_angle_Z ) );
    const LoKi::LorentzVector B_PXYZE( B_P * B_cosangle_X, B_P * B_cosangle_Y,
                                       B_P * B_cosangle_Z,
                                       ::sqrt( B_P * B_P + B_M * B_M ) );

    // Bc daughters momenta.
    ...
    const double bp =
        bx * mu_PXYZE.X() + by * mu_PXYZE.Y() + bz * mu_PXYZE.Z();
    const double gamma2 = b2 > 0 ? ( gamma - 1. ) / b2 : 0.;

    // Muon momentum in the Bd rest frame.
    const double mu_PXYZE_Brestframe_X =
        mu_PXYZE.X() + gamma2 * bp * bx + gamma * bx * mu_PXYZE.E();
    const double mu_PXYZE_Brestframe_Y =
        mu_PXYZE.Y() + gamma2 * bp * by + gamma * by * mu_PXYZE.E();
    const double mu_PXYZE_Brestframe_Z =
        mu_PXYZE.Z() + gamma2 * bp * bz + gamma * bz * mu_PXYZE.E();
    const double mu_PXYZE_Brestframe_E = gamma * ( mu_PXYZE.E() + bp );
    const LoKi::LorentzVector mu_PXYZE_Brestframe(
        mu_PXYZE_Brestframe_X, mu_PXYZE_Brestframe_Y, mu_PXYZE_Brestframe_Z,
        mu_PXYZE_Brestframe_E );

    // Discriminant variables.
    const double              El     = mu_PXYZE_Brestframe.E();
    const LoKi::LorentzVector Pcomb1 = B_PXYZE - D_PXYZE - mu_PXYZE;
    const double Mmiss2 = Pcomb1.E() * Pcomb1.E() - Pcomb1.X() * Pcomb1.X() -
                          Pcomb1.Y() * Pcomb1.Y() - Pcomb1.Z() * Pcomb1.Z();
    const LoKi::LorentzVector Pcomb2 = B_PXYZE - D_PXYZE;
    const double q2 = Pcomb2.E() * Pcomb2.E() - Pcomb2.X() * Pcomb2.X() -
	    Pcomb2.Y() * Pcomb2.Y() - Pcomb2.Z() * Pcomb2.Z();
    const double Lxy = ::sqrt(B_direction.X()*B_direction.X() + B_direction.Y()*B_direction.Y());		
    const double CTAU = Lxy*B_M/::sqrt(B_PXYZE.X()*B_PXYZE.X() + B_PXYZE.Y()*B_PXYZE.Y());
    const double TAU = CTAU*10/2.998; // Gives lifetime in picoseconds
    ...
    test &= tuple->column( "FitVar_FlightDist" , norm );
    test &= tuple->column( "FitVar_Lxy" , Lxy );
    test &= tuple->column( "FitVar_CTAU" , CTAU );
    test &= tuple->column( "FitVar_TAU" , TAU );
...
//=========================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fillVertexFull(
    const LHCb::VertexBase* vtx, const LHCb::Particle* P,
    const std::string& prefix, const std::string& vtx_name,
    Tuples::Tuple& tuple ) const {
  if ( !vtx )
    ++counter( "Can't retrieve the " + vtx_name + " vertex for " + prefix );
  ...
  if ( sc.isFailure() )
    Warning( "Error in fillVertexFull " + prefix, sc, 1 ).ignore();
  return sc;
}
...
//=========================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fillBPV( const VertexBase*  primVtx,
                                             const Particle*    P,
  ...
  if ( !test )
    Warning( "Error in fillBPV " + prefix, StatusCode( test ), 1 ).ignore();
  return StatusCode( test );
}
//=========================================================================
//  Fill PV for all PV
//=========================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fillMinIP( const Particle*    P,
                                               const std::string& prefix,
                                               Tuples::Tuple& tuple ) const {
  bool test = true;
  // minimum IP
  ...
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "PV size: " << PV.size() << endmsg;
  ...
  if ( isVerbose() ) {
    if ( msgLevel( MSG::VERBOSE ) ) {
      verbose() << "Filling IP " << prefix + "_MINIP " << ipmin << " at "
                << minchi2 << endmsg;
      verbose() << "Filling IP next best " << prefix + "_MINIP "
                << ipminnextbest << " at " << minchi2nextbest << endmsg;
    }
...
// fill vertex stuff
//=========================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fillVertex( const LHCb::VertexBase* vtx,
...
//=========================================================================
StatusCode TupleToolTauMuDiscrVars_RJpsi::fillFlight(
    const VertexBase* oriVtx, const Particle* P, const std::string& prefix,
...
// =====================================================
const VertexBase* TupleToolTauMuDiscrVars_RJpsi::originVertex(
