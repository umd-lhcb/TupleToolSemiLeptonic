...
//=============================================================================
TupleToolDira::TupleToolDira(const std::string& type, const std::string& name,
...
StatusCode TupleToolDira::initialize() {
  ...
  if (!m_context)
    return Error("Could not get parent DVAlgorithm", StatusCode::FAILURE);
...
StatusCode TupleToolDira::fill(const Particle* mother, const Particle* P,
...
StatusCode TupleToolDira::fillDIRAError(const VertexBase* primVtx,
  ...
  bool test = true;
  test &= tuple->column(head + "_DiraAngleError",
                        diraerr / (std::sqrt(1 - dira * dira)));
...
void TupleToolDira::calculateDIRAError(const Particle* P,
                                       const VertexBase* primVtx, double& dira,
  ...
  const Gaudi::XYZVector vectord =
      P->endVertex()->position() - primVtx->position();
