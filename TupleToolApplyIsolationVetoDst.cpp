...
*/
  ...
  for (LHCb::Particle::ConstVector::const_iterator itmp = bDaughters.begin();
    ...
    if ((*itmp)->particleID().abspid() == 421) theD = (*itmp);
  }
    ...
    for (LHCb::Particle::Range::const_iterator iparts = (parts).begin();
      ...
      if (part->proto()->track()->type() < 5 &&
        ...
        if (part->proto()->track()->type() == 3) {
          ...
          //       warning() << "Dstar mass"<< newP->momentum().M() << " D mass
          //       " << theD->momentum().M() <<  "deltaMass " << deltaMass << "
          //       fit stats " <<  sc4 << endmsg;
        }
        //     warning() << "deltaMass " << deltaMass << " track type " <<
        //     part->proto()->track()->type() << endr
...
void TupleToolApplyIsolationVetoDst::writeParticle(
    const LHCb::Particle* P, double bdt, std::string name, Tuples::Tuple& tuple,
    ...
    if (m_trueID) {
      ...
      if (P) {
        ...
        for (std::vector<IParticle2MCAssociator*>::const_iterator iMCAss =
          ...
          if (!foundMCP) mcp = (*iMCAss)->relatedMCP(P);
          if (mcp) {
            foundMCP = true;
          }
          if (!foundMCP_mother) mother_mcp = (*iMCAss)->relatedMCP(Mother);
          if (mother_mcp) {
