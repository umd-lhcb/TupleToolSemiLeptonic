...
//     warning() << "deltaMass " << deltaMass << " track type " << part->proto()->track()->type() << endr
    ...
    if(deltaMassWindow){
      ...
      if(bdtval > DstBDT1){DstBDT2=DstBDT1;DstM2=DstM1;DstBDT1=bdtval;DstM1=deltaMass;}
      else if (bdtval > DstBDT2){DstBDT2=bdtval;DstM2=deltaMass;}
      continue;
    }
	...
	//StatusCode dump2 = m_dist->distance((const LHCb::Particle *) part,(const LHCb::Vertex *)vd,D_ip,D_chi2);
	    ...
	    if(chi2 < 50){
	        ...
	        if (bdtval > maxbdt[i]) {
	            ...
	            SelParts[i]=part;maxbdt[i]=bdtval;
	            break;
	        }
	    }
     }
...
//  
//=========================================================================
const Vertex* TupleToolApplyIsolationVetoDst::originVertex( const Particle* top
...
void TupleToolApplyIsolationVetoDst::writeParticle(const  LHCb::Particle* P,double bdt, std::string name, Tuples::Tuple& tuple, std::string prefix,const  LHCb::Particle* Mother){
    ...
    float charge = 0;
    if(P->proto()->track()->type() > 2){charge=P->proto()->track()->charge();}
    ...
    if(m_trueID){
	  ...
	  if(!foundMCP)
	    mcp = (*iMCAss)->relatedMCP(P);
          if ( mcp ) {foundMCP=true;}
          if(!foundMCP_mother)
	    mother_mcp= (*iMCAss)->relatedMCP(Mother);
          if ( mother_mcp ) {foundMCP_mother=true;}
        }
	  ...
	  float motherID=-1;
      if(foundMCP) if(mcp->mother()) motherID = mcp->mother()->particleID().abspid();
	  ...
	  if(mother_mcp){
		...
		if( (head == mother_mcp) || (head->key()==mother_mcp->key())) sameMother=1;
	      }
