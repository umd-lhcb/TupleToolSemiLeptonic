...
// local
#include "TupleToolConeIsolationWithTruthMatching.h"
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolConeIsolationWithTruthMatching
//
// 2015-02-19 : Simone Bifani, Michel De Cian
// 2016-03-10 : Adlene Hicheur
// 2017-01-20 : Julian Garcia Pardinas --> Added pi0 truth matching
//-----------------------------------------------------------------------------
#define MAXNPIZEROS 1000
// Declaration of the Algorithm Factory
DECLARE_TOOL_FACTORY(TupleToolConeIsolationWithTruthMatching)

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
TupleToolConeIsolationWithTruthMatching::
    TupleToolConeIsolationWithTruthMatching(const std::string &type,
  ...
  declareProperty(
      "SizeStep", m_sizeStep = 0.1,
      "Set the cone size step between two iterations (default = 0.1)");
  declareProperty(
      "MinhPi0Mass", m_MinhPi0Mass = 0 * Gaudi::Units::MeV,
      "Set the lower seed-Pi0 invariant mass cut (default = 0 MeV)");
  declareProperty(
      "MaxhPi0Mass", m_MaxhPi0Mass = 5000 * Gaudi::Units::MeV,
      "Set the upper seed-Pi0 invariant mass cut (default = 5000 MeV)");
  declareProperty("FillCharged", m_fillCharged = true,
                  "Flag to fill the charged cone (default = true)");
  declareProperty("ExtraParticlesLocation",
                  m_extraParticlesLocation = "Phys/StdAllNoPIDsMuons/Particles",
                  "Set the type of particles that are considered in the "
                  "charged cone (default = Phys/StdAllNoPIDsMuons/Particles)");
  declareProperty(
      "TrackType", m_trackType = 3,
  ...
  declareProperty("ExtraPhotonsLocation",
                  m_extraPhotonsLocation = "Phys/StdLooseAllPhotons/Particles",
                  "Set the type of particles that are considered in the "
                  "neutral cone (default = Phys/StdLooseAllPhotons/Particles)");
  ...
  declareProperty("PizerosLocation",
                  m_PizerosLocation = "Phys/StdLoosePi02gg/Particles",
                  "Set the type of pi0s that are considered (default = "
                  "Phys/StdLoosePi02gg/Particles)");
  declareProperty("MergedPizerosLocation",
                  m_MergedPizerosLocation = "Phys/StdLooseMergedPi0/Particles",
                  "Set the type of merged pi0s that are considered (default = "
  ...
  declareProperty("FillMaxPt", m_fillMaxPt = true,
                  "Flag to fill the momentum of the max-pT object in the cone "
                  "(default = true)");
  declareProperty("MaxPtParticlesLocation",
                  m_maxPtParticleLocation = "Phys/StdAllLooseMuons/Particles",
                  "Set the type of max-pT particles that are considered in the "
                  "charged cone (default = Phys/StdAllLooseMuons/Particles)");

  declareProperty("FillComponents", m_fillComponents = false,
                  "Flag to fill the components of all relevant variables "
                  "(default = false)");
...
// Initialization
//=============================================================================
StatusCode TupleToolConeIsolationWithTruthMatching::initialize() {
  StatusCode sc = TupleToolBase::initialize();
  if (sc.isFailure()) return sc;

  if (!m_fillCharged && !m_fillNeutral) {
    if (msgLevel(MSG::FATAL)) fatal() << "No cone to fill" << endmsg;
...
// Fill the tuple
//=============================================================================
StatusCode TupleToolConeIsolationWithTruthMatching::fill(
    const LHCb::Particle *top, const LHCb::Particle *seed,
    const std::string &head, Tuples::Tuple &tuple) {
  const std::string prefix = fullName(head);

  if (msgLevel(MSG::DEBUG)) debug() << "==> Fill" << endmsg;

  // -- The vector m_decayParticles contains all the particles that belong to
  // the given decay  according to the decay descriptor.
  ...
  // -- Save all particles that belong to the given decay in the vector
  // m_decayParticles
  ...
  if (seed) {
    ...
    while (coneSize <= m_maxConeSize) {
      ...
      if (m_fillCharged) {
        if (parts->size() != 0) {
          ...
          StatusCode ccSc =
              ChargedCone(seed, parts, maxPts, coneSize, ccMultiplicity,
                          ccScalarPt, ccMomentum, ccMaxPt);
          if (ccSc.isFailure()) ccMultiplicity = -1;
          ...
          if (ccDeltaPhi > M_PI) ccDeltaPhi = 2 * M_PI - ccDeltaPhi;

          // -- Fill the tuple with the variables
          test &= tuple->column(prefix + "_" + conesize + "_cc_mult",
                                ccMultiplicity);
          test &=
              tuple->column(prefix + "_" + conesize + "_cc_sPT", ccScalarPt);
          test &= tuple->column(prefix + "_" + conesize + "_cc_vPT",
                                sqrt(ccP.Perp2()));
          ...
          if (m_fillAsymmetry) {
            test &= tuple->column(prefix + "_" + conesize + "_cc_asy_P",
                                  (seed->p() - sqrt(ccP.Mag2())) /
                                      (seed->p() + sqrt(ccP.Mag2())));
            test &= tuple->column(prefix + "_" + conesize + "_cc_asy_PT",
                                  (seed->pt() - sqrt(ccP.Perp2())) /
                                      (seed->pt() + sqrt(ccP.Perp2())));
            if (m_fillComponents) {
              test &= tuple->column(prefix + "_" + conesize + "_cc_asy_PX",
                                    (seed->momentum().Px() - ccP.X()) /
                                        (seed->momentum().Px() + ccP.X()));
              test &= tuple->column(prefix + "_" + conesize + "_cc_asy_PY",
                                    (seed->momentum().Py() - ccP.Y()) /
                                        (seed->momentum().Py() + ccP.Y()));
              test &= tuple->column(prefix + "_" + conesize + "_cc_asy_PZ",
                                    (seed->momentum().Pz() - ccP.Z()) /
                                        (seed->momentum().Pz() + ccP.Z()));
            }
          }

          if (m_fillDeltas) {
            test &= tuple->column(prefix + "_" + conesize + "_cc_deltaEta",
                                  ccDeltaEta);
            test &= tuple->column(prefix + "_" + conesize + "_cc_deltaPhi",
                                  ccDeltaPhi);
          }

          if (m_fillIsolation)
            test &= tuple->column(
                prefix + "_" + conesize + "_cc_IT",
                seed->pt() / sqrt(pow(seed->momentum().Px() + ccP.X(), 2) +
                                  pow(seed->momentum().Py() + ccP.Y(), 2)));

          if (m_fillMaxPt) {
            test &= tuple->column(prefix + "_" + conesize + "_cc_maxPt_Q",
                                  (int)ccMaxPt[4]);
            test &=
                tuple->column(prefix + "_" + conesize + "_cc_maxPt_PT",
                              sqrt(pow(ccMaxPt[0], 2) + pow(ccMaxPt[1], 2)));
            if (m_fillComponents) {
              test &= tuple->column(prefix + "_" + conesize + "_cc_maxPt_PX",
                                    ccMaxPt[0]);
              test &= tuple->column(prefix + "_" + conesize + "_cc_maxPt_PY",
                                    ccMaxPt[1]);
              test &= tuple->column(prefix + "_" + conesize + "_cc_maxPt_PZ",
                                    ccMaxPt[2]);
              test &= tuple->column(prefix + "_" + conesize + "_cc_maxPt_PE",
                                    ccMaxPt[3]);
            }
          }
        } else {
          ...
          if (m_fillMaxPt) {
            ...
            if (m_fillComponents) {
              test &=
                  tuple->column(prefix + "_" + conesize + "_cc_maxPt_PX", 0.);
              test &=
                  tuple->column(prefix + "_" + conesize + "_cc_maxPt_PY", 0.);
              test &=
                  tuple->column(prefix + "_" + conesize + "_cc_maxPt_PZ", 0.);
              test &=
                  tuple->column(prefix + "_" + conesize + "_cc_maxPt_PE", 0.);
            }
          }
        }
      }
      ...
      if (m_fillNeutral) {
        if (photons->size() != 0) {
          ...
          StatusCode ncSc =
              NeutralCone(seed, photons, coneSize, ncMultiplicity, ncScalarPt,
                          ncMomentum, ncMaxPt, avConfLevel, maxConfLevel, LegH1,
                          LegH2, LegH3, LegH4, ncTruthMatch);
          if (ncSc.isFailure()) ncMultiplicity = -1;
          ...
          if (ncDeltaPhi > M_PI) ncDeltaPhi = 2 * M_PI - ncDeltaPhi;

          // -- Fill the tuple with the variables
          test &= tuple->column(prefix + "_" + conesize + "_nc_mult",
                                ncMultiplicity);
          test &= tuple->column(prefix + "_" + conesize + "_nc_TruthMatch",
                                ncTruthMatch);
          test &=
              tuple->column(prefix + "_" + conesize + "_nc_sPT", ncScalarPt);
          test &= tuple->column(prefix + "_" + conesize + "_nc_vPT",
                                sqrt(ncP.Perp2()));
          ...
          if (m_fillComponents) {
            ...
            test &= tuple->column(prefix + "_" + conesize + "_nc_avCL",
                                  avConfLevel);
            test &= tuple->column(prefix + "_" + conesize + "_nc_maxCL",
                                  maxConfLevel);
          }

          if (m_fillAsymmetry) {
            test &= tuple->column(prefix + "_" + conesize + "_nc_asy_P",
                                  (seed->p() - sqrt(ncP.Mag2())) /
                                      (seed->p() + sqrt(ncP.Mag2())));
            test &= tuple->column(prefix + "_" + conesize + "_nc_asy_PT",
                                  (seed->pt() - sqrt(ncP.Perp2())) /
                                      (seed->pt() + sqrt(ncP.Perp2())));
            if (m_fillComponents) {
              test &= tuple->column(prefix + "_" + conesize + "_nc_asy_PX",
                                    (seed->momentum().Px() - ncP.X()) /
                                        (seed->momentum().Px() + ncP.X()));
              test &= tuple->column(prefix + "_" + conesize + "_nc_asy_PY",
                                    (seed->momentum().Py() - ncP.Y()) /
                                        (seed->momentum().Py() + ncP.Y()));
              test &= tuple->column(prefix + "_" + conesize + "_nc_asy_PZ",
                                    (seed->momentum().Pz() - ncP.Z()) /
                                        (seed->momentum().Pz() + ncP.Z()));
            }
          }

          if (m_fillDeltas) {
            test &= tuple->column(prefix + "_" + conesize + "_nc_deltaEta",
                                  ncDeltaEta);
            test &= tuple->column(prefix + "_" + conesize + "_nc_deltaPhi",
                                  ncDeltaPhi);
          }

          if (m_fillIsolation)
            test &= tuple->column(
                prefix + "_" + conesize + "_nc_IT",
                seed->pt() / sqrt(pow(seed->momentum().Px() + ncP.X(), 2) +
                                  pow(seed->momentum().Py() + ncP.Y(), 2)));

          if (m_fillMaxPt) {
            test &=
                tuple->column(prefix + "_" + conesize + "_nc_maxPt_PT",
                              sqrt(pow(ncMaxPt[0], 2) + pow(ncMaxPt[1], 2)));
            if (m_fillComponents) {
              test &= tuple->column(prefix + "_" + conesize + "_nc_maxPt_PX",
                                    ncMaxPt[0]);
              test &= tuple->column(prefix + "_" + conesize + "_nc_maxPt_PY",
                                    ncMaxPt[1]);
              test &= tuple->column(prefix + "_" + conesize + "_nc_maxPt_PZ",
                                    ncMaxPt[2]);
              ...
              //      test &= tuple->column(prefix + "_" + conesize +
              //      "_nc_Mass",TheMass);
            }
          }
        } else {
          ...
          if (m_fillMaxPt) {
            ...
            if (m_fillComponents) {
              test &=
                  tuple->column(prefix + "_" + conesize + "_nc_maxPt_PX", 0.);
              test &=
                  tuple->column(prefix + "_" + conesize + "_nc_maxPt_PY", 0.);
              test &=
                  tuple->column(prefix + "_" + conesize + "_nc_maxPt_PZ", 0.);
      ...
      if (m_fillIsolation) {
        if (m_fillCharged && m_fillNeutral) {
          if ((parts->size() != 0) && (photons->size() != 0))
            test &= tuple->column(
                prefix + "_" + conesize + "_IT",
                seed->momentum().Pt() /
                    sqrt(pow(seed->momentum().Px() + ccP.X() + ncP.X(), 2) +
                         pow(seed->momentum().Py() + ccP.Y() + ncP.Y(), 2)));
          else
    ...
    if (m_fillPi0Info) {
      if (pizeros->size() != 0) {
        ...
        std::vector<double> MassWithPi0, Pi0MomX, Pi0MomY, Pi0MomZ, Pi0MomE,
            Pi0MomM, Pi0DeltaR, Pi0Ph1CL, Pi0Ph2CL;
        std::vector<int> Pi0Truth;
        StatusCode pizSc = pi0Comb(seed, pizeros, Pi0Multiplicity, MassWithPi0,
                                   Pi0MomX, Pi0MomY, Pi0MomZ, Pi0MomE, Pi0MomM,
                                   Pi0DeltaR, Pi0Ph1CL, Pi0Ph2CL, Pi0Truth);
        // -- Fill the tuple with the variables
        test &= tuple->column(prefix + "_pi0_mult", Pi0Multiplicity);
        test &= tuple->farray(prefix + "_MasshPi0", MassWithPi0.begin(),
                              MassWithPi0.end(), prefix + "_Pi0_ArrayLength",
                              MAXNPIZEROS);
        test &=
            tuple->farray(prefix + "_Pi0_PX", Pi0MomX.begin(), Pi0MomX.end(),
                          prefix + "_Pi0_ArrayLength", MAXNPIZEROS);
        test &=
            tuple->farray(prefix + "_Pi0_PY", Pi0MomY.begin(), Pi0MomY.end(),
                          prefix + "_Pi0_ArrayLength", MAXNPIZEROS);
        test &=
            tuple->farray(prefix + "_Pi0_PZ", Pi0MomZ.begin(), Pi0MomZ.end(),
                          prefix + "_Pi0_ArrayLength", MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0_E", Pi0MomE.begin(), Pi0MomE.end(),
                              prefix + "_Pi0_ArrayLength", MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0_M", Pi0MomM.begin(), Pi0MomM.end(),
                              prefix + "_Pi0_ArrayLength", MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0_DeltaR", Pi0DeltaR.begin(),
                              Pi0DeltaR.end(), prefix + "_Pi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0Ph1_CL", Pi0Ph1CL.begin(),
                              Pi0Ph1CL.end(), prefix + "_Pi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0Ph2_CL", Pi0Ph2CL.begin(),
                              Pi0Ph2CL.end(), prefix + "_Pi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_Pi0_TruthMatch", Pi0Truth.begin(),
                              Pi0Truth.end(), prefix + "_Pi0_ArrayLength",
                              MAXNPIZEROS);
      } else {
        ...
        std::vector<int> Pi0Truth = {0};

        test &= tuple->column(prefix + "_pi0_mult", 0);
        test &=
            tuple->farray(prefix + "_MasshPi0", MassWithPi0.begin(),
                          MassWithPi0.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_PX", Pi0MomX.begin(),
                              Pi0MomX.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_PY", Pi0MomY.begin(),
                              Pi0MomY.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_PZ", Pi0MomZ.begin(),
                              Pi0MomZ.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_E", Pi0MomE.begin(), Pi0MomE.end(),
                              prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_M", Pi0MomM.begin(), Pi0MomM.end(),
                              prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_DeltaR", Pi0MomE.begin(),
                              Pi0MomE.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0Ph1_CL", Pi0Ph1CL.begin(),
                              Pi0Ph1CL.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0Ph2_CL", Pi0Ph2CL.begin(),
                              Pi0Ph2CL.end(), prefix + "_Pi0_ArrayLength", 0);
        test &= tuple->farray(prefix + "_Pi0_TruthMatch", Pi0Truth.begin(),
                              Pi0Truth.end(), prefix + "_Pi0_ArrayLength", 0);
      }
    }
    ...
    if (m_fillMergedPi0Info) {
      if (mergedpizeros->size() != 0) {
        ...
        std::vector<double> MassWithMergPi0, Pi0MomX, Pi0MomY, Pi0MomZ, Pi0MomE,
            Pi0MomM, Pi0DeltaR, Pi0Ph1CL, Pi0Ph2CL;
        std::vector<int> Pi0Truth;
        StatusCode mpizSc =
            pi0Comb(seed, mergedpizeros, MergPi0Multiplicity, MassWithMergPi0,
                    Pi0MomX, Pi0MomY, Pi0MomZ, Pi0MomE, Pi0MomM, Pi0DeltaR,
                    Pi0Ph1CL, Pi0Ph2CL, Pi0Truth);
        // -- Fill the tuple with the variables
        test &= tuple->column(prefix + "_mergpi0_mult", MergPi0Multiplicity);
        test &= tuple->farray(prefix + "_MasshMergPi0", MassWithMergPi0.begin(),
                              MassWithMergPi0.end(),
                              prefix + "_MergPi0_ArrayLength", MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0_PX", Pi0MomX.begin(),
                              Pi0MomX.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0_PY", Pi0MomY.begin(),
                              Pi0MomY.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0_PZ", Pi0MomZ.begin(),
                              Pi0MomZ.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &=
            tuple->farray(prefix + "_MergPi0_E", Pi0MomE.begin(), Pi0MomE.end(),
                          prefix + "_MergPi0_ArrayLength", MAXNPIZEROS);
        test &=
            tuple->farray(prefix + "_MergPi0_M", Pi0MomM.begin(), Pi0MomM.end(),
                          prefix + "_MergPi0_ArrayLength", MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0_DeltaR", Pi0DeltaR.begin(),
                              Pi0DeltaR.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0Ph1_CL", Pi0Ph1CL.begin(),
                              Pi0Ph1CL.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0Ph2_CL", Pi0Ph2CL.begin(),
                              Pi0Ph2CL.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
        test &= tuple->farray(prefix + "_MergPi0_TruthMatch", Pi0Truth.begin(),
                              Pi0Truth.end(), prefix + "_MergPi0_ArrayLength",
                              MAXNPIZEROS);
      } else {
        std::vector<int> MassWithMergPi0 = {-1};
        std::vector<double> Pi0MomX = {-1e6};
        std::vector<double> Pi0MomY = {-1e6};
        std::vector<double> Pi0MomZ = {-1e6};
        std::vector<double> Pi0MomE = {-1e6};
        ...
        test &= tuple->farray(prefix + "_MasshMergPi0", MassWithMergPi0.begin(),
                              MassWithMergPi0.end(),
                              prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_PX", Pi0MomX.begin(),
                          Pi0MomX.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_PY", Pi0MomY.begin(),
                          Pi0MomY.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_PZ", Pi0MomZ.begin(),
                          Pi0MomZ.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_E", Pi0MomE.begin(), Pi0MomE.end(),
                          prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_M", Pi0MomM.begin(), Pi0MomM.end(),
                          prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_DeltaR", Pi0MomE.begin(),
                          Pi0MomE.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0Ph1_CL", Pi0Ph1CL.begin(),
                          Pi0Ph1CL.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0Ph2_CL", Pi0Ph2CL.begin(),
                          Pi0Ph2CL.end(), prefix + "_MergPi0_ArrayLength", 0);
        test &=
            tuple->farray(prefix + "_MergPi0_TruthMatch", Pi0Truth.begin(),
                          Pi0Truth.end(), prefix + "_MergPi0_ArrayLength", 0);
      }
    }
...
//=============================================================================
void TupleToolConeIsolationWithTruthMatching::saveDecayParticles(
...
//=============================================================================
StatusCode TupleToolConeIsolationWithTruthMatching::ChargedCone(
    const LHCb::Particle *seed, const LHCb::Particles *parts,
    const LHCb::Particles *maxPts, const double rcut, int &multiplicity,
    double &scalarPt, std::vector<double> &momentum,
  ...
  for (LHCb::Particles::const_iterator ip = parts->begin(); ip != parts->end();
       ++ip) {
    ...
    if (proto) {
      ...
      if (track) {
        ...
        // -- Calculate the difference in Eta and Phi between the seed particle
        // and a track
          ...
          if (track->type() == m_trackType) {
            ...
            if (maxPts->size() != 0) {
              if (track->pt() > sqrt(pow(maxPtPx, 2) + pow(maxPtPy, 2))) {
                for (LHCb::Particles::const_iterator iPt = maxPts->begin();
                     iPt != maxPts->end(); ++iPt) {
...
//=============================================================================
StatusCode TupleToolConeIsolationWithTruthMatching::NeutralCone(
    const LHCb::Particle *seed, const LHCb::Particles *photons,
    const double rcut, int &multiplicity, double &scalarPt,
    std::vector<double> &momentum, std::vector<double> &maxPt,
    double &AvConfLevel, double &MaxConfLevel, double &LegendreRat1,
    double &LegendreRat2, double &LegendreRat3, double &LegendreRat4,
    int &ConeTruthMatching) {
  // -- Initialize values
  multiplicity = 0;
  scalarPt = 0;
  ConeTruthMatching = 0;
  ...
  // -- Find the MC Eve associated to the seed particle
  const LHCb::MCParticle *mcseed(NULL);
  for (std::vector<IParticle2MCAssociator *>::const_iterator iMCAss =
           m_p2mcAssocs.begin();
       iMCAss != m_p2mcAssocs.end(); ++iMCAss) {
    mcseed = (*iMCAss)->relatedMCP(seed);
    if (mcseed) break;
  }

  for (LHCb::Particles::const_iterator ip = photons->begin();
       ip != photons->end(); ++ip) {
    ...
    // -- Calculate the difference in Eta and Phi between the seed particle and
    // a photons
    ...
    if ((rcut == 0) || (deltaR < rcut)) {
      ...
      // -- Check if the photon is truth matched
      ...
      if (mcseed && mcphoton) {
        if (mcseed->mother() && mcphoton->mother() &&
            abs(mcphoton->particleID().pid()) == 22) {
          if (mcphoton->mother()->mother() &&
              abs(mcphoton->mother()->particleID().pid()) == 111) {
            if (abs(mcseed->mother()->particleID().pid()) ==
                abs(mcphoton->mother()->mother()->particleID().pid())) {
              ConeTruthMatching = 1;
            }
          }
        }
      ...
      numLeg1 +=
          std::sqrt(photonMomentum.Mag2()) * gsl_sf_legendre_Pl(1, cosAng);
      numLeg2 +=
          std::sqrt(photonMomentum.Mag2()) * gsl_sf_legendre_Pl(2, cosAng);
      numLeg3 +=
          std::sqrt(photonMomentum.Mag2()) * gsl_sf_legendre_Pl(3, cosAng);
      numLeg4 +=
          std::sqrt(photonMomentum.Mag2()) * gsl_sf_legendre_Pl(4, cosAng);

      // Extra Photon
      if (sqrt(photonMomentum.Perp2()) >
          sqrt(pow(maxPtPx, 2) + pow(maxPtPy, 2))) {
...
//=============================================================================
bool TupleToolConeIsolationWithTruthMatching::isTrackInDecay(
    const LHCb::Track *track) {
  bool isInDecay = false;

  for (std::vector<const LHCb::Particle *>::iterator ip =
           m_decayParticles.begin();
       ip != m_decayParticles.end(); ++ip) {
...
//=============================================================================
StatusCode TupleToolConeIsolationWithTruthMatching::pi0Comb(
    const LHCb::Particle *seed, const LHCb::Particles *pi0s, int &multiplicity,
    std::vector<double> &InvMass, std::vector<double> &Pi0PX,
    std::vector<double> &Pi0PY, std::vector<double> &Pi0PZ,
    std::vector<double> &Pi0E, std::vector<double> &Pi0M,
    std::vector<double> &Pi0DeltaR, std::vector<double> &Pi0Photon1CL,
    std::vector<double> &Pi0Photon2CL, std::vector<int> &Pi0TruthMatching) {
  // -- Initialize values
  ...
  // -- Find the MC particle associated to the seed particle
  const LHCb::MCParticle *mcseed(NULL);
  for (std::vector<IParticle2MCAssociator *>::const_iterator iMCAss =
           m_p2mcAssocs.begin();
       iMCAss != m_p2mcAssocs.end(); ++iMCAss) {
  ...
  for (LHCb::Particles::const_iterator ip = pi0s->begin(); ip != pi0s->end();
       ++ip) {
    ...
    if (masshPi0 < m_MaxhPi0Mass && masshPi0 > m_MinhPi0Mass) {
      ...
      // Get Pi0 truth information:
      const LHCb::MCParticle *mcpion(NULL);
      for (std::vector<IParticle2MCAssociator *>::const_iterator iMCAss =
               m_p2mcAssocs.begin();
           iMCAss != m_p2mcAssocs.end(); ++iMCAss) {
        mcpion = (*iMCAss)->relatedMCP(pion0);
        if (mcpion) break;
      }
      ...
      if (mcseed && mcpion) {
        if (mcseed->mother()) {
          if (mcpion->mother()) {
            if ((abs(mcpion->particleID().pid()) == 111) &&
                (abs(mcseed->mother()->particleID().pid()) ==
                 abs(mcpion->mother()->particleID().pid()))) {
              isTruthMatched = 1;
            }
          }
        }
      }
      Pi0TruthMatching.push_back(isTruthMatched);
      ...
      if (!(pion0->isBasicParticle())) {
        ...
        for (SmartRefVector<LHCb::Particle>::const_iterator iph =
                 photons.begin();
             iph != photons.end(); ++iph) {
