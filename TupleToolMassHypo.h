 ...
 * KS0_M_with_piplus_p~minus, KS0_M_with_pplus_piminus,
 KS0_M_with_pplus_p~minus.
 *
 * Cases with 3 or more daughters are possible but not tested.
 *
 * Warnings:
 *
 * 1) PIDReplacement is a python dictionary. To test two mass hypthesis
 replacements
...
class TupleToolMassHypo : public TupleToolBase,
                          virtual public IParticleTupleTool {
 public:
  /// Standard constructor
  TupleToolMassHypo(const std::string& type, const std::string& name,
 ...
 private:
  double recalculateMass(const possDecay&, const Gaudi::XYZPoint&,
                         std::string& combname);
