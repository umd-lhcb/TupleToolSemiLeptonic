...
TupleToolDecayTreeFitter::TupleToolDecayTreeFitter(const std::string& type,
  ...
  declareProperty("StoreRefittedPVsTwice", m_storeAnyway = false,
                  "Store PV even if a refitted version is already the best PV "
                  "(i.e store twice)");
...
StatusCode TupleToolDecayTreeFitter::initialize() {
  ...
  for (std::vector<std::string>::const_iterator iterS =
           m_massConstraints.begin();
  ...
  if (0 == m_dva)
    return Error("Couldn't get parent IDVAlgorithm", StatusCode::FAILURE);
  ...
  if (m_extraName.empty()) {
    ...
    if ("TupleToolDecayTreeFitter" == m_extraName)
      m_extraName = "";  // user has not chanegd instance name
    info() << "All fields will be prepended with ``" << m_extraName << "''"
           << endmsg;
  }

  if (m_extraName.empty()) {
    return Error(
        "Extraname is empty. Always give an instance name "
...
StatusCode TupleToolDecayTreeFitter::fill(const LHCb::Particle* mother,
  ...
  if (P->isBasicParticle()) {
    return Error(
        "Do not call TupleToolDecayTreeFitter for basic particles. Use "
        "Branches. See doxygen.");
  }
  const std::string prefix = fullName(head);
  if (msgLevel(MSG::DEBUG))
    debug() << "head ''" << head << "'' prefix ''" << prefix
            << "'' extraname ''" << m_extraName << "''" << endmsg;

  const ITrackStateProvider* stateprovider =
      (m_stateprovider.empty() ? NULL : &(*m_stateprovider));

  LHCb::DecayTree tree(*P);
  // substitute
  if (m_substitute && !substitute(tree)) return StatusCode::FAILURE;
  if (!m_massConstraintsPids.empty() && !checkMassConstraints(tree))
  ...
  if (m_constrainToOriginVertex) {
    ...
    for (std::vector<const VertexBase*>::const_iterator iv = originVtx.begin();
         iv != originVtx.end(); ++iv) {
      if (msgLevel(MSG::DEBUG))
        debug() << "Creating DecayTreeFitter on " << tree.head() << " " << *iv
                << endmsg;
      DecayTreeFitter::Fitter fitter(*(tree.head()), *(*iv), stateprovider);
      if (msgLevel(MSG::DEBUG)) debug() << "Created DecayTreeFitter" << endmsg;
      if (!fit(fitter, tree.head(), *iv, prefix, tMap))
        return StatusCode::FAILURE;
    }
  } else {
    if (msgLevel(MSG::DEBUG))
      debug() << "Do not constrain the origin vertex" << endmsg;
    // Get the fitter
...
StatusCode TupleToolDecayTreeFitter::fit(DecayTreeFitter::Fitter& fitter,
  ...
  if (msgLevel(MSG::VERBOSE))
    verbose() << "fit " << P << " " << pv << " " << prefix << endmsg;
  bool test = true;
  // add mass contraints
  if (!m_massConstraintsPids.empty()) {
    for (std::vector<LHCb::ParticleID>::const_iterator iterC =
             m_massConstraintsPids.begin();
...
// Fill standard stuff
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillDecay(
    const DecayTreeFitter::Fitter& fitter, const std::string& prefix,
    TupleMap& tMap) const {
  bool test = true;
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillMomentum(
    const DecayTreeFitter::Fitter& fitter, const Particle* P,
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillLT(
    const DecayTreeFitter::Fitter& fitter, const Particle* P,
    const std::string& prefix, TupleMap& tMap) const {
  bool test = true;
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillDaughters(
    const DecayTreeFitter::Fitter& fitter, const LHCb::Particle* P,
    const std::string& prefix, TupleMap& tMap) const {
  bool test = true;
  ...
  if (msgLevel(MSG::DEBUG))
    debug() << "for id " << P->particleID().pid() << " daughter size is "
  ...
  for (LHCb::Particle::ConstVector::const_iterator it = daughters.begin();
    ...
    while (usedNames.find(name) != usedNames.end()) {  // fix to bug 88702
      renamed = true;
      if (msgLevel(MSG::VERBOSE))
        verbose() << "Found already name " << name << " trying next " << endmsg;
      name =
          prefix + "_" + pidName + "_" + boost::lexical_cast<std::string>(add);
      ++add;
    }
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillStableDaughters(
    const DecayTreeFitter::Fitter& fitter, const LHCb::Particle* P,
    const std::string& prefix, TupleMap& tMap) const {
  bool test = true;

  if (msgLevel(MSG::VERBOSE))
    verbose() << "FillStableDaughters " << prefix << endmsg;
  ...
  for (LHCb::Particle::ConstVector::const_iterator it = daughters.begin();
    ...
    if (!particle->isBasicParticle()) {
      ...
      while (usedNames.find(name) != usedNames.end()) {  // fix to bug 88702
        if (msgLevel(MSG::VERBOSE))
          verbose() << "Found already name " << name << " trying next "
                    << endmsg;
        renamed = true;
      ...
      while (usedNames.find(name) != usedNames.end()) {  // fix to bug 88702
        if (msgLevel(MSG::VERBOSE))
          verbose() << "Found already name " << name << " trying next "
                    << endmsg;
        renamed = true;
        name = prefix + "_" + pidName + "_" +
               boost::lexical_cast<std::string>(add);
        ++add;
      }
      if (renamed)
        Info("Renaming duplicate to " + name, StatusCode::SUCCESS, 1);
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillTracksMomentum(
    const DecayTreeFitter::Fitter& fitter, const Particle* P,
    const std::string& prefix, TupleMap& tMap) const {
  bool test = true;

  if (msgLevel(MSG::VERBOSE))
    verbose() << "FillTracksMomentum " << prefix << endmsg;
...
StatusCode TupleToolDecayTreeFitter::insert(const std::string& leaf,
  ...
  if (msgLevel(MSG::VERBOSE))
    verbose() << "insert " << leaf << " " << val << " size " << l->second.size()
              << endmsg;
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::fillTuple(
    const TupleMap& tMap, Tuples::Tuple& tuple,
...
// Sort Tracks
//=============================================================================
std::set<const LHCb::Track*> TupleToolDecayTreeFitter::sortedTracks(
    const LHCb::VertexBase* vb) const {
...
bool TupleToolDecayTreeFitter::samePV(const LHCb::VertexBase* vb1,
  ...
  if (!vb1 && !vb2) {
    Warning(
        "samePV method called with 2 NULL PVs. "
        "The answer is obviously true, but you may want to check the meaning "
        "of the question.",
        StatusCode::SUCCESS, 1);
    return true;
  } else if (!vb1 || !vb2) {
    Warning(
        "samePV method called with 1 NULL PV. "
        "The answer is obviously false, but you may want to check the meaning "
        "of the question.",
        StatusCode::SUCCESS, 1);
    return false;
  }
  ...
  const bool inc =
      std::includes(st1.begin(), st1.end(), st2.begin(), st2.end());
  if (msgLevel(MSG::VERBOSE)) {
...
// get origin vertex
//=============================================================================
std::vector<const VertexBase*> TupleToolDecayTreeFitter::originVertex(
    const Particle* mother, const Particle* P) const {
  ...
  if (mother == P) {  // the origin vertex is the primary.
    ...
    if (bpv) {
      oriVx.push_back(bpv);
      if (msgLevel(MSG::VERBOSE))
        verbose() << "Pushed back bpv " << bpv << " from " << tesLocation(bpv)
                  << " at " << bpv->position() << endmsg;
    } else if (m_constrainToOriginVertex) {
      Warning(
          "NULL bestPV while constraining to origin vertex. Fit will be "
          "ignored.",
    ...
    for (LHCb::RecVertex::Range::const_iterator pv =
             m_dva->primaryVertices().begin();
         pv != m_dva->primaryVertices().end(); ++pv) {
      if (m_storeAnyway || !samePV(*pv, bpv)) {
        oriVx.push_back(*pv);
        if (msgLevel(MSG::VERBOSE))
          verbose() << "Pushed back  pv " << *pv << " from " << tesLocation(*pv)
                    << " at " << (*pv)->position() << endmsg;
      }
...
StatusCode TupleToolDecayTreeFitter::substitute(LHCb::DecayTree& tree) {
  ...
  if (msgLevel(MSG::VERBOSE) || 0 == substituted) {
    ...
    for (LHCb::DecayTree::CloneMap::const_iterator i = mp.begin();
         i != mp.end(); ++i) {
      if (i->first->particleID().pid() == i->second->particleID().pid()) {
        info() << "A " << getName(i->first->particleID().pid())
               << " remains unchanged" << endmsg;
      } else {
        info() << "A " << getName(i->first->particleID().pid())
               << " is substituted by a "
  ...
  if (0 == substituted) {
    return Error(
        "No particles have been substituted. Check your substitution options.");
  }
  return StatusCode::SUCCESS;
}
//=============================================================================
...
//=============================================================================
StatusCode TupleToolDecayTreeFitter::checkMassConstraints(
    const LHCb::DecayTree& tree) {
  if (!m_first) return StatusCode::SUCCESS;  // do that only once
  m_first = false;
  const LHCb::DecayTree::CloneMap mp = tree.cloneMap();
  for (std::vector<LHCb::ParticleID>::const_iterator m =
           m_massConstraintsPids.begin();
    ...
    if (found && msgLevel(MSG::VERBOSE))
      verbose() << "Constraint " << getName(m->pid()) << " was found in tree"
                << endmsg;
    if (!found) {
      std::ostringstream mess;
      mess << "Constraint " << getName(m->pid())
           << " was not found in tree. Check your options. Maybe also the "
