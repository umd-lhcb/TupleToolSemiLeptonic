...
TupleToolCaloHypo::TupleToolCaloHypo(const std::string& type,
  ...
  declareProperty("AllowDaughtersHypo",
                  m_daugh = false);  // SplitPhotons from merged
...
StatusCode TupleToolCaloHypo::initialize() {
  ...
  m_estimator =
      tool<ICaloHypoEstimator>("CaloHypoEstimator", "CaloHypoEstimator", this);
  m_tables = tool<ICaloRelationsGetter>("CaloRelationsGetter",
                                        "CaloRelationsGetter", this);
  m_caloElectron = tool<ICaloElectron>("CaloElectron", this);
  m_pileup = tool<ICaloDigitFilterTool>("CaloDigitFilterTool", "FilterTool");

  info() << "Caution : this TupleTool may trigger part of Calo reconstruction "
            "- ONLY WORK ON DST"
         << endmsg;
...
StatusCode TupleToolCaloHypo::fill(const Particle*, const Particle* P,
  ...
  for (SmartRefVector<CaloHypo>::const_iterator ih = hypos.begin();
       hypos.end() != ih; ++ih) {
    if (!isPureNeutralCalo(P)) {
      if ((*ih)->hypothesis() == LHCb::CaloHypo::EmCharged)
        hypo = *ih;
      else
        hypo2 = *ih;
    } else
  ...
  if (hypo != NULL && hypo->hypothesis() == LHCb::CaloHypo::Pi0Merged &&
      m_daugh) {
  ...
  // info() << "coucou 1 " << hypo << " " << hypo2 << " " << daugh1 << " " <<
  // daugh2 << endmsg;
  ...
  if (m_charged)
    filltuple &= tuple->column(prefix + "_CaloHypo_DeltaTheta", dT);
  if (hypo != NULL && hypo->hypothesis() == LHCb::CaloHypo::Pi0Merged &&
      m_daugh && daugh1 != NULL && daugh2 != NULL) {
  ...
  const LHCb::CaloCluster* cl =
      LHCb::CaloAlgUtils::ClusterFromHypo(hypo, false);
  ...
  for (int id = 0; id < Last; ++id) {
    ...
    if (useData(id, mask)) {
      double val = (NULL != hypo && hasCalo)
                       ? m_estimator->data(hypo, (DataType)id, 0.)
                       : 0.;
    ...
    if (P->charge() != 0 && m_brem && useData(id, 0x1)) {
      double bval = (NULL != hypo2 && hasBremCalo)
                        ? m_estimator->data(hypo2, (DataType)id, 0.)
                        : 0.;
      // info() << "coucou loop 2 " << Name[id] << " " << bval << endmsg;
      filltuple &= tuple->column(prefix + "_BremCaloHypo_" + Name[id], bval);
    }
    if (hypo != NULL && hypo->hypothesis() == LHCb::CaloHypo::Pi0Merged &&
        m_daugh && useData(id, 0x1)) {
      double val0 =
          (NULL != daugh1) ? m_estimator->data(daugh1, (DataType)id, 0.) : 0.;
      double val1 =
          (NULL != daugh2) ? m_estimator->data(daugh2, (DataType)id, 0.) : 0.;
      // info() << "coucou loop 3 " << Name[id] << " " << val0 << " " << val1 <<
      // endmsg;
...
bool TupleToolCaloHypo::useData(int id, int mask) {
  ...
  for (std::vector<std::string>::iterator i = m_dataList.begin();
       m_dataList.end() != i; ++i) {
...
double TupleToolCaloHypo::incidence(const LHCb::CaloHypo* hypo,
                                    bool straight) const {
  const LHCb::CaloCluster* cluster =
      LHCb::CaloAlgUtils::ClusterFromHypo(hypo, true);
  std::string m_cmLoc =
      LHCb::CaloAlgUtils::CaloIdLocation("ClusterMatch", context());
  ...
  if (LHCb::CaloHypo::EmCharged == hypo->hypothesis() && !straight) {
    ...
    if (exist<LHCb::Calo2Track::IClusTrTable>(m_cmLoc)) {
      LHCb::Calo2Track::IClusTrTable* ctable =
          m_tables->getClusTrTable(m_cmLoc);
      const LHCb::Calo2Track::IClusTrTable::Range range =
          ctable->relations(cluster);
      if (!range.empty()) {
        ...
        if (m_caloElectron->set(proto))
          incidence = m_caloElectron->caloState().momentum().Theta();
        delete proto;
      }
    }
  } else {
