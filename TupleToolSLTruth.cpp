...
TupleToolSLTruth::TupleToolSLTruth(const std::string& type,
    ...
    : TupleToolBase(type, name, parent), m_toolList(1, "MCTupleToolKinematic") {
  // interface
...
StatusCode TupleToolSLTruth::getMCTruePi0s(const LHCb::MCParticle* MCP,
  ...
  if (MCP) {
    if (abs(MCP->particleID().pid()) == 111) {
      ...
      if (!MCP->endVertices().empty() && abs(MCP->particleID().pid()) > 100 &&
        ...
        if (MCPendV) {
          ...
          if (!MCPdaughters.empty()) {
            ...
            for (itDMCP = MCPdaughters.begin(); MCPdaughters.end() != itDMCP;
                 ++itDMCP) {
              if ((*itDMCP)) {
                getMCTrueGammas((*itDMCP), MCTrueGammaPX_, MCTrueGammaPY_,
                                MCTrueGammaPZ_, MCTrueGammaE_, MCTrueGammaMID_);
              }
            }
          }
        }
...
StatusCode TupleToolSLTruth::fill(const LHCb::Particle*,
  ...
  std::vector<int> MCTruePi0MID, MCTrueGammaMID;
  ...
  if (mcp) {
    if (!mcp->endVertices().empty()) {
      if (abs(mcp->particleID().pid()) == 511 ||
          abs(mcp->particleID().pid()) == 521 ||
          abs(mcp->particleID().pid()) == 531 ||
  ...
  for (unsigned int i = 0; i < 3; i++) {
    ...
    if (hadronvectors_D.size() > i) {
      ...
      test &= tuple->column(prefix + "_TrueHadron_D" + sout.str() + "_ID", 0);
    }
  }
  for (unsigned int j = 0; j < 3; j++) {
    ...
    if (hadronvectors_D1GD.size() > j) {
      ...
      test &= tuple->column(prefix + "_TrueHadron_D0_GD" + sout1.str() + "_ID",
                            hadronids_D1GD[j]);
    } else {
      test &= tuple->column(prefix + "_TrueHadron_D0_GD" + sout1.str() + "_P",
                            Gaudi::LorentzVector(0, 0, 0, 0));
      test &=
          tuple->column(prefix + "_TrueHadron_D0_GD" + sout1.str() + "_ID", 0);
    }
  }
  for (unsigned int j = 0; j < 3; j++) {
    ...
    if (hadronvectors_D2GD.size() > j) {
      ...
      test &= tuple->column(prefix + "_TrueHadron_D1_GD" + sout2.str() + "_ID",
                            hadronids_D2GD[j]);
    } else {
      test &= tuple->column(prefix + "_TrueHadron_D1_GD" + sout2.str() + "_P",
                            Gaudi::LorentzVector(0, 0, 0, 0));
      test &=
          tuple->column(prefix + "_TrueHadron_D1_GD" + sout2.str() + "_ID", 0);
    }
  }
  for (unsigned int j = 0; j < 3; j++) {
    ...
    if (hadronvectors_D3GD.size() > j) {
      ...
      test &= tuple->column(prefix + "_TrueHadron_D2_GD" + sout3.str() + "_ID",
                            hadronids_D3GD[j]);
    } else {
      test &= tuple->column(prefix + "_TrueHadron_D2_GD" + sout3.str() + "_P",
                            Gaudi::LorentzVector(0, 0, 0, 0));
      test &=
          tuple->column(prefix + "_TrueHadron_D2_GD" + sout3.str() + "_ID", 0);
    }
  }
  ...
  test &=
      tuple->farray(prefix + "_MCTrue_pi0_PX", MCTruePi0PX_copy,
                    prefix + "_MCTrue_pi0_ArrayLength", max_neutral_candidates);
  test &=
      tuple->farray(prefix + "_MCTrue_pi0_PY", MCTruePi0PY_copy,
                    prefix + "_MCTrue_pi0_ArrayLength", max_neutral_candidates);
  test &=
      tuple->farray(prefix + "_MCTrue_pi0_PZ", MCTruePi0PZ_copy,
                    prefix + "_MCTrue_pi0_ArrayLength", max_neutral_candidates);
  test &=
      tuple->farray(prefix + "_MCTrue_pi0_E", MCTruePi0E_copy,
                    prefix + "_MCTrue_pi0_ArrayLength", max_neutral_candidates);
  test &=
      tuple->farray(prefix + "_MCTrue_pi0_mother_ID", MCTruePi0MID_copy,
                    prefix + "_MCTrue_pi0_ArrayLength", max_neutral_candidates);
  test &= tuple->column(prefix + "_MCTrue_gamma_mult", MCTrueGammaMultiplicity);
  ...
  test &= tuple->farray(prefix + "_MCTrue_gamma_E", MCTrueGammaE_copy,
                        prefix + "_MCTrue_gamma_ArrayLength",
                        max_neutral_candidates);
  test &= tuple->farray(prefix + "_MCTrue_gamma_mother_ID", MCTrueGammaMID_copy,
                        prefix + "_MCTrue_gamma_ArrayLength",
                        max_neutral_candidates);
