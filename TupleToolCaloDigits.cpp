...
TupleToolCaloDigits::TupleToolCaloDigits(const std::string& type,
  ...
  if (m_auto_configure) {
    ...
    fatal() << "Invalid CaloName. Mus be one of  Spd,Prs,Ecal,Hcal if "
               "AutoConfigure is used (i.e. true)"
            << endmsg;
  }
}
//=============================================================================
...
StatusCode TupleToolCaloDigits::fill(Tuples::Tuple& tuple) {
  ...
  for (const auto& digit : *digits) {
    ...
    if (msgLevel(MSG::DEBUG))
      debug() << cellID.toString() << " has an energy of " << m_es.back()
              << " \n";
  }
  if (msgLevel(MSG::DEBUG))
    debug() << " saved " << m_index.size()
            << " digits to n tuple " + m_CaloName + "Digit." << endmsg;
  tuple->farray(m_extraName + m_CaloName + "DigitIndex", std::begin(m_index),
                std::end(m_index), m_extraName + m_CaloName + "DigitNi",
                m_maxSize);
  m_index.clear();
  tuple->farray(m_extraName + m_CaloName + "DigitCalo", std::begin(m_calo),
                std::end(m_calo), m_extraName + m_CaloName + "DigitNc",
                m_maxSize);
  m_calo.clear();
  tuple->farray(m_extraName + m_CaloName + "DigitArea", std::begin(m_area),
                std::end(m_area), m_extraName + m_CaloName + "DigitNa",
                m_maxSize);
  m_area.clear();
  tuple->farray(m_extraName + m_CaloName + "DigitRow", std::begin(m_row),
                std::end(m_row), m_extraName + m_CaloName + "DigitNr",
                m_maxSize);
  m_row.clear();
  tuple->farray(m_extraName + m_CaloName + "DigitColumn", std::begin(m_column),
                std::end(m_column), m_extraName + m_CaloName + "DigitNco",
                m_maxSize);
  m_column.clear();
  tuple->farray(m_extraName + m_CaloName + "Digit_X", std::begin(m_xs),
                std::end(m_xs), m_extraName + m_CaloName + "DigitNx",
                m_maxSize);
  m_xs.clear();
  tuple->farray(m_extraName + m_CaloName + "Digit_Y", std::begin(m_ys),
                std::end(m_ys), m_extraName + m_CaloName + "DigitNy",
                m_maxSize);
  m_ys.clear();
  tuple->farray(m_extraName + m_CaloName + "Digit_Z", std::begin(m_zs),
                std::end(m_zs), m_extraName + m_CaloName + "DigitNz",
                m_maxSize);
  m_zs.clear();
  tuple->farray(m_extraName + m_CaloName + "DigitEnergy", std::begin(m_es),
                std::end(m_es), m_extraName + m_CaloName + "DigitNe",
                m_maxSize);
  m_es.clear();
