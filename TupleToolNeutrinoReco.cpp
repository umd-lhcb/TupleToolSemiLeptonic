...
TupleToolNeutrinoReco::TupleToolNeutrinoReco(const std::string& type,
    ...
    : TupleToolBase(type, name, parent), m_dva(NULL) {
...
StatusCode TupleToolNeutrinoReco::fill(const LHCb::Particle*,
  ...
  if (P) {
    ...
    if (isVerbose()) {
      ...
      double a =
          4. * (Nu_Perp * Nu_Perp + P->measuredMass() * P->measuredMass());
      double X =
          4. * Nu_Parl *
          (2 * Nu_Perp * Nu_Perp + P->measuredMass() * P->measuredMass());
...
double TupleToolNeutrinoReco::MomentumParallel(
    const LHCb::VertexBase* pv, const LHCb::Particle* b,
...
double TupleToolNeutrinoReco::MomentumPerpendicular(
    const LHCb::VertexBase* pv, const LHCb::Particle* b,
    const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> >* mv) {
  if (b->endVertex() == NULL || pv == NULL) return 0.;
  double fd_x = b->endVertex()->position().X() - pv->position().X();
...
double TupleToolNeutrinoReco::MomentumQuadratic(const double Parl,
                                                ...
                                                const double Mass) {
  return -0.5 * Parl *
         (1. - (m_motherMass * m_motherMass - Perp * Perp) /
                   (Perp * Perp + Mass * Mass));
}

double TupleToolNeutrinoReco::MomentumSqrt(const double Parl, const double Perp,
                                           const double Mass) {
  double a = 4. * (Perp * Perp + Mass * Mass);
  double b = 4. * Parl *
             (2. * Perp * Perp - m_motherMass * m_motherMass + Mass * Mass);
  double c = 4. * Perp * Perp * (Parl * Parl + m_motherMass * m_motherMass) -
             (m_motherMass * m_motherMass - Mass * Mass) *
                 (m_motherMass * m_motherMass - Mass * Mass);

  double d = b * b - 4. * a * c;
  if (d > 0.)
    return std::sqrt(d) / (2. * a);
  else
    return 0;
}
