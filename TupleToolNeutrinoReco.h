 ...
 *
 * - head_Nu_Parl : momentum of the input particle paralell to the direction of
 * flight
 * - head_Nu_Perp : momentum of the input particle perpendicular to the
 * direction of flight
 *
 ...
 * overall mass rescaling is required the remaining calculation can be done as
 * follows:
 *
 ...
 * c = B0_Nu_Y + 4*B0_Nu_Perp*B0_Nu_Perp*mB*mB - (mB*mB - B0_MM*B0_MM)*(mB*mB -
 * B0_MM*B0_MM)
 *
 * pb = -B0_Nu_Parl*( 1 - (mB*mB-B0_Nu_Perp*B0_Nu_Perp)/(B0_Nu_Perp*B0_Nu_Perp +
 * B0_MM*B0_MM)  )/2 pd = sqrt(b*b - 4*B0_Nu_a*c)/(2*B0_Nu_a)
 *
...
class TupleToolNeutrinoReco : public TupleToolBase,
 ...
 public:
  ...
  TupleToolNeutrinoReco(const std::string& type, const std::string& name,
                        const IInterface* parent);

  ~TupleToolNeutrinoReco() {}  ///< Destructor
 ...
 public:
  ...
  double MomentumParallel(
      const LHCb::VertexBase*, const LHCb::Particle*,
      const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> >*);

  double MomentumPerpendicular(
      const LHCb::VertexBase*, const LHCb::Particle*,
      const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> >*);

  double MomentumQuadratic(const double, const double, const double);

  double MomentumSqrt(const double, const double, const double);
